...
void dumpPackedTree(ofstream & myfile, char * node ){

    if ( *node == LEAF_TAG ){
        node++;
        Node_Leaf * leaf=(Node_Leaf * ) node;
        myfile<< "Leaf "<< leaf->x_val<<" "<<leaf->y_val<<" "<<leaf->corr;
        return;
    }
    else if( *node == INNER_TAG ){
        node++;
        Node_Inner_NoIndirection * inner=(Node_Inner_NoIndirection * ) node;
        
        myfile<< "Node "<< inner->splitAxis <<" "<<inner->splitLoc<<
        " "<<inner->min_x<<" "<<inner->max_x<<" "<<inner->min_y<<" "<<inner->max_y<<" ";
