...
-- Work: O(1)
-- Span: O(1)
v_isEmpty :: Vector a -> Bool
v_isEmpty vec = v_length vec == 0
...
v_generate_loop :: Vector a -> Int -> Int -> (Int -> a) -> ()
v_generate_loop vec start end f =
    if start == end
    then ()
    else
      let _ = v_unsafe_update vec start (f start)
      in v_generate_loop vec (start+1) end f

-- Work: O(n)
-- Span: O(n)
v_generate :: Int -> (Int -> a) -> Vector a
v_generate n f =
    let vec = v_alloc n
        n'  = max n 0
...
v_generate_par_loop :: Vector a -> Int -> Int -> (Int -> a) -> Int -> ()
v_generate_par_loop vec start end f cutoff =
    if (end - start) <= cutoff
    then v_generate_loop vec start end f
    else
      let mid = div (start + end) 2
          _   = spawn (v_generate_par_loop vec start mid f cutoff)
          _   = v_generate_par_loop vec mid end f cutoff
          _   = sync
      in ()

-- Work: O(n)
-- Span: O(1)
v_generate_par :: Int -> (Int -> a) -> Vector a
v_generate_par n f =
    let vec = v_alloc n
        n'  = max n 0
        cutoff = 1000
        _ = v_generate_par_loop vec 0 n f 1000
    in vec
