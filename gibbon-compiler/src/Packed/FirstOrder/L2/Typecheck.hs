...
data TCError ex = GenericTC String ex
                | VarNotFoundTC Var ex
                | UnsupportedExpTC ex
                | DivergingEffectsTC ex LocationTypeState LocationTypeState
                | LocationTC String ex LocVar LocVar
                | ModalityTC String ex LocVar LocationTypeState
                deriving (Read,Show,Eq,Ord, Generic, NFData)
...
type TcM ex a = Except (TCError ex) a
...
tcExp ddfs env funs constrs regs tstatein exp@(L _ ex) =
    ...
    case ex of
      ...
      AppE v ls e ->
          ...
          do let (ArrowTy{arrIn,arrOut,locVars}) = getFunTy v funs
...
ensureEqual :: Eq a => ex -> String -> a -> a -> TcM ex a
...
checkLen :: (Show op, Show arg) => ex -> op -> Int -> [arg] -> TcM ex ()
...
-- Includes an expression for error reporting.
ensureEqualTy :: (Eq ty, Show ty) => ex -> ty -> ty -> TcM ex ty
ensureEqualTy exp a b = ensureEqual exp ("Expected these types to be the same: "
                                         ++ (show a) ++ ", " ++ (show b)) a b


tcProj :: (Show ty) => ex -> Int -> (UrTy ty) -> TcM ex (UrTy ty)
tcProj _ i (ProdTy tys) = return $ tys !! i
tcProj e _i ty = throwError $ GenericTC ("Projection from non-tuple type "
                                         ++ (show ty)) e


-- | Insert a region into a region set.
-- Includes an expression for error reporting.
regionInsert :: ex -> Region -> RegionSet -> TcM ex RegionSet
...
-- Includes an expression for error reporting.
ensureRegion :: ex -> Region -> RegionSet -> TcM ex ()
...
-- Includes an expression for error reporting.
getRegion :: ex -> ConstraintSet -> LocVar -> TcM ex Region
...
-- Includes an expression for error reporting.
lookupVar :: Env2 ty -> Var -> ex -> TcM ex ty
...
-- things.
-- TODO: Fix this!
combineTStates :: ex -> LocationTypeState -> LocationTypeState -> TcM ex LocationTypeState
...
-- Includes an expression for error reporting.
ensureEqualTyNoLoc :: (Eq ty, Show ty) => ex -> (UrTy ty) -> (UrTy ty) -> TcM ex (UrTy ty)
...
-- Includes an expression for error reporting.
ensureMatchCases :: (Out ty) => DDefs (UrTy ty) -> ex -> UrTy ty -> [(DataCon, [(Var,LocVar)], ex)] -> TcM ex ()
...
-- Includes an expression for error reporting.
ensurePackedLoc :: (Eq ty, Show ty) => Exp -> UrTy ty -> ty -> TcM Exp ()
...
-- Includes an expression for error reporting.
ensureDataCon :: ex -> LocVar -> [Ty2] -> ConstraintSet -> TcM ex ()
...
-- Includes an expression for error reporting.
ensureAfterConstant :: ex -> ConstraintSet -> LocVar -> LocVar -> TcM ex ()
...
-- Includes an expression for error reporting.
ensureAfterPacked :: ex -> ConstraintSet -> LocVar -> LocVar -> TcM ex ()
...
_lookupTS :: Exp -> LocVar -> LocationTypeState -> TcM Exp (Modality,Bool)
...
switchOutLoc :: Exp -> LocationTypeState -> LocVar -> TcM Exp LocationTypeState
...
_absentAfter :: Exp -> LocationTypeState -> LocVar -> TcM Exp ()
...
absentStart :: Exp -> ConstraintSet -> Region -> TcM Exp ()
...
removeLoc :: Exp -> LocationTypeState -> LocVar -> TcM Exp LocationTypeState
