...
freshNames (L1.Prog defs funs main) =
    ...
    where freshFuns m = M.fromList <$> mapM freshFun (M.toList m)
          ...
          freshExp vs (L1.E1 (L1.ProjE i e)) =
              do e' <- freshExp vs e
                 return $ L1.E1 $ L1.ProjE i e'
          freshExp vs (L1.E1 (L1.MkProdE es)) =
              do es' <- mapM (freshExp vs) es
          ...
          freshExp vs (L1.E1 (L1.MapE (v,t,b) e)) =
              do b' <- freshExp vs b
                 e' <- freshExp vs e
                 return $ L1.E1 $ L1.MapE (v,t,b') e'
          freshExp vs (L1.E1 (L1.FoldE (v1,t1,e1) (v2,t2,e2) e3)) =
              do e1' <- freshExp vs e1
                 e2' <- freshExp vs e2
                 e3' <- freshExp vs e3
