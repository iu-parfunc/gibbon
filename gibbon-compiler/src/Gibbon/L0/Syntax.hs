...
data E0Ext loc dec
  = LambdaE
      [(Var, dec)] -- Variable tagged with type
      (PreExp E0Ext loc dec)
  | PolyAppE
      (PreExp E0Ext loc dec) -- Operator
...
instance Out Loc.Pos
 where
  docPrec _ pos = doc pos
  doc (Loc.Pos path line col _) =
    hcat [doc path, colon, doc line, colon, doc col]


--------------------------------------------------------------------------------
-- Instances for E0Ext
instance (Out l, Out d) => FreeVars (E0Ext l d) where
  gFreeVars e =
    case e of
...
instance (Out l, Out d, Show l, Show d) => Expression (E0Ext l d)
 where
  type LocOf (E0Ext l d) = l
  type TyOf (E0Ext l d)  = d
  isTrivial _            = False

instance (Show l, Out l) => Flattenable (E0Ext l Ty0)
 where
  gFlattenGatherBinds _ddfs _env ex = return ([], ex)
  gFlattenExp _ddfs _env ex         = return ex

instance HasSubstitutableExt E0Ext l d =>
         SubstitutableExt (PreExp E0Ext l d) (E0Ext l d) where
...
instance (Out l, Out d) => FreeVars (LinearExt l d) where
  gFreeVars e =
    case e of
...
instance (Out l, Out d, Show l, Show d) => Expression (LinearExt l d)
 where
  type LocOf (LinearExt l d) = l
  type TyOf (LinearExt l d)  = d
  isTrivial _                = False

instance (Show l, Out l) => Flattenable (LinearExt l Ty0)
 where
  gFlattenGatherBinds _ddfs _env ex = return ([], ex)
  gFlattenExp _ddfs _env ex         = return ex

instance HasSubstitutableExt E0Ext l d =>
         SubstitutableExt (PreExp E0Ext l d) (LinearExt l d) where
  gSubstExt old new ext =
    case ext of
  ...
  gSubstEExt old new ext =
    case ext of
      ReverseAppE fn arg ->
        ReverseAppE (gSubstE old new fn) (gSubstE old new arg)
...
recoverType ddfs env2 ex =
  case ex of
    VarE v ->
      M.findWithDefault
        (error $ "recoverType: Unbound variable " ++ show v)
        v
        (vEnv env2)
    LitE _ -> IntTy
    CharE _ -> CharTy
    ...
    LitSymE _ -> IntTy
    AppE v tyapps _ ->
      let (ForAll tyvars (ArrowTy _ retty)) = fEnv env2 # v
    ...
    CaseE _ mp ->
      let (c, args, e) = head mp
       ...
       in recoverType
            ddfs
            (extendsVEnv (M.fromList (zip args' (lookupDataCon ddfs c))) env2)
            e
      ...
      case ext of
        ...
        FunRefE _ f ->
          case (M.lookup f (vEnv env2), M.lookup f (fEnv env2)) of
            (Nothing, Nothing) ->
              error $ "recoverType: Unbound function " ++ show f
            (Just ty, _) -> ty
        ...
        LinearExt lin ->
          case lin of
            ReverseAppE fn _args ->
              case recoverType ddfs env2 fn of
                ArrowTy _ ty -> ty
                oth ->
                  error $
                  "recoverType: ReverseAppE expected a function type, got: " ++
                  sdoc oth
            LseqE _ b -> recoverType ddfs env2 b
            AliasE a ->
              let ty = recoverType ddfs env2 a
               in ProdTy [ty, ty]
