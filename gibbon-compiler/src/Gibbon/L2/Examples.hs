...
add1TraversedFun = FunDef "add1" ["tr1"] add1TraversedFunTy add1FunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where add1TraversedFunTy = add1FunTy { arrEffs = S.fromList [Traverse "lin2"] }


add1Fun :: FunDef2
add1Fun = FunDef "add1" ["tr1"] add1FunTy add1FunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
...
id1Fun :: FunDef2
id1Fun = FunDef "id1" ["tr18"] idFunTy idFunBod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
    idFunBod = VarE "tr18"
...
copyTreeFun :: FunDef2
copyTreeFun = FunDef "copyTree" ["tr22"] copyFunTy copyBod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
    copyFunTy = ArrowTy2
...
id2Fun :: FunDef2
id2Fun = FunDef "id2" ["tr41"] id2Ty id2Bod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
    id2Ty :: ArrowTy2 Ty2
    id2Ty = ArrowTy2
...
id3Fun :: FunDef2
id3Fun = FunDef "id3" ["i42"] id3Ty id3Bod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
    id3Ty :: ArrowTy2 Ty2
    id3Ty = ArrowTy2
             []
...
intAddFun :: FunDef2
intAddFun = FunDef "intAdd" ["i109"] intAddTy id3Bod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
leftmostFun :: FunDef2
leftmostFun = FunDef "leftmost" ["t111"] leftmostTy leftmostBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
rightmostFun = FunDef "rightmost" ["t242"] rightmostTy rightmostBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
buildLeafFun = FunDef "buildLeaf" ["i125"] buildLeafTy buildLeafBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
buildTreeFun = FunDef "buildTree" ["i270"] buildTreeTy buildTreeBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
buildTwoTreesFun = FunDef "buildTwoTrees" ["i750"] buildTreeTy buildTreeBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
    buildTreeTy :: ArrowTy2 Ty2
    buildTreeTy = ArrowTy2
...
buildTreeSumFun = FunDef "buildTreeSum" ["i302"] buildTreeSumTy buildTreeSumBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
sumTreeFun :: FunDef2
sumTreeFun = FunDef "sumTree" ["tr762"] sumTreeTy sumTreeBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
addTreesFun :: FunDef2
addTreesFun = FunDef "addTrees" ["trees354"] addTreesTy addTreesBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
testProdFun :: FunDef2
testProdFun = FunDef "testprod" ["tup130"] testprodTy testprodBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
    testprodTy = ArrowTy2
...
sumUpFun :: FunDef2
sumUpFun = FunDef "sumUp" ["tr1"] sumUpFunTy sumUpFunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
valueSTreeFun = FunDef "valueSTree" ["tr522"] valueSTreeFunTy valueSTreeFunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
buildSTreeFun = FunDef "buildSTree" ["i543"] buildSTreeTy buildSTreeBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
sumSTreeFun :: FunDef2
sumSTreeFun = FunDef "sumSTree" ["tr762"] sumSTreeTy sumSTreeBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
evenFun :: FunDef2
evenFun = FunDef "even" ["i560"] evenFunTy evenFunBod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
setEvenFun :: FunDef2
setEvenFun = FunDef "setEven" ["tr570"] setEvenFunTy setEvenFunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
sumUpSetEvenFun = FunDef "sumUpSetEven" ["tr600"] sumUpSetEvenFunTy sumUpSetEvenFunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
copyExprFun :: FunDef2
copyExprFun = FunDef "copyExpr" ["e700"] copyExprFunTy copyExprFunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
substFun :: FunDef2
substFun = FunDef "subst" ["tr653"] substFunTy substFunBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
indrBuildTreeFun = FunDef "indrBuildTree" ["i270"] indrBuildTreeTy indrBuildTreeBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
indrRightmostFun = FunDef "indrRightmost" ["t742"] indrRightmostTy indrRightmostBod (FunMeta Rec NoInline False NoLayoutOpt Nothing Nothing)
  where
...
indrIDFun :: FunDef2
indrIDFun = FunDef "indrID" ["tr800"] indrIDTy indrIDBod (FunMeta NotRec NoInline False NoLayoutOpt Nothing Nothing)
  where
