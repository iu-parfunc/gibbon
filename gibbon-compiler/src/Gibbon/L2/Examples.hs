...
tTypeable :: Exp2
tTypeable =  Ext $ LetRegionE (VarR "r500") $
...
add1FunBod = CaseE (VarE "tr1") $
  ...
  , ("Node", [("x9","l10"),("y11","l12")],
     ...
     Ext $ LetLocE "l15" (AfterVariableLE "x14" "l13") $
...
add1MainExp :: Exp2
add1MainExp = Ext $ LetRegionE (VarR "r99") $
...
leafMainExp :: Exp2
leafMainExp = Ext $ LetRegionE (VarR "r150") $
...
-- writes node
nodeMainExp :: Exp2
nodeMainExp = Ext $ LetRegionE (VarR "r155") $
               ...
               Ext $ LetLocE "l159" (AfterVariableLE "x158" "l157") $
  ...
  where
    ...
    copyBod = CaseE (VarE "tr22") $
                 ...
                 , ("Node", [("x29","lx30"),("y31","ly32")],
                    ...
                    Ext  $ LetLocE "ly35" (AfterVariableLE "x34" "lx33") $
...
copyTreeMainExp = Ext $ LetRegionE (VarR "r200") $
...
copyOnId1MainExp = Ext $ LetRegionE (VarR "r220") $
...
leftmostMainExp = Ext $ LetRegionE (VarR "r122") $
                  ...
                  Ext $ LetLocE "l126" (AfterVariableLE "x125" "l124") $
...
rightmostMainExp = Ext $ LetRegionE (VarR "r253") $
                   ...
                   Ext $ LetLocE "l257" (AfterVariableLE "x256" "l255") $
...
buildLeafMainExp = Ext $ LetRegionE (VarR "r128") $
  ...
  where
    ...
    buildTreeBod = LetE ("b279",[], BoolTy, PrimAppE EqIntP [VarE "i270", LitE 0]) $
                   ...
                   (LetE ("i273",[], IntTy, PrimAppE SubP [VarE "i270", LitE 1]) $
                    ...
                    Ext $ LetLocE "l276" (AfterVariableLE "x275" "l274") $
...
buildTreeMainExp = Ext $ LetRegionE (VarR "r279") $
...
buildTwoTreesMainExp = Ext $ LetRegionE (VarR "r756") $
                       Ext $ LetLocE "l757" (StartOfLE (VarR "r756")) $
                       Ext $ LetRegionE (VarR "r758") $
  ...
  where
    ...
    buildTreeSumBod = LetE ("b303",[], BoolTy, PrimAppE EqIntP [VarE "i302", LitE 0]) $
                      ...
                      (LetE ("i303",[], IntTy, PrimAppE SubP [VarE "i302", LitE 1]) $
                       ...
                       Ext $ LetLocE "l306" (AfterVariableLE "x305" "l304") $
...
buildTreeSumMainExp = Ext $ LetRegionE (VarR "r313") $
...
sumTreeMainExp :: Exp2
sumTreeMainExp = Ext $ LetRegionE (VarR "r771") $
...
printTupMainExp = Ext $ LetRegionE (VarR "r325") $
                    ...
                    ("Node", [("x360","l361"), ("y362","l363")],
                     CaseE (VarE "tree2")
                        [("Node", [("x364","l365"), ("y366","l367")],
                          ...
                          Ext $ LetLocE "l370" (AfterVariableLE "x369" "l368") $
...
addTreesMainExp = Ext $ LetRegionE (VarR "r400") $
                  Ext $ LetLocE "l401" (StartOfLE (VarR "r400")) $
                  LetE ("x402",[], PackedTy "Tree" "l401",
                           AppE "buildTree" ["l401"] [LitE 2]) $
                  -- Ext $ LetLocE "l403" (AfterVariableLE "x402" "l401") $
                  Ext $ LetRegionE (VarR "r403") $
                  Ext $ LetLocE "l403" (StartOfLE (VarR "r403")) $
                  LetE ("y404",[], PackedTy "Tree" "l403",
                           AppE "buildTree" ["l403"] [LitE 2]) $
                  LetE ("z405",[], ProdTy [PackedTy "Tree" "l401", PackedTy "Tree" "l403"],
                           MkProdE [VarE "x402", VarE "y404"]) $
                  Ext $ LetRegionE (VarR "r405") $
    ...
    testprodBod = LetE ("t134",[], PackedTy "Tree" "lin131", ProjE 0 (VarE "tup130")) $
                  ...
                  [("Leaf",[("n136","l137")],
                   ...
                   ("Node",[("x140","l141"), ("y142","l143")],
                    ...
                    Ext $ LetLocE "l146" (AfterVariableLE "x149" "l144") $
  ...
  where
    ...
    sumUpFunBod = CaseE (VarE "tr1") $
      ...
      , ("Inner", [("i506","l507"),("b508","l509"),("x510","l511"),("y512","l513")],
         ...
         Ext $ LetLocE "l550" (AfterVariableLE "i506" "l514") $
         Ext $ LetLocE "l551" (AfterVariableLE "b508" "l550") $
         LetE ("x515",[],PackedTy "STree" "l551",
                   AppE "sumUp" ["l511","l551"] [VarE "x510"]) $
         Ext $ LetLocE "l516" (AfterVariableLE "x515" "l551") $
  ...
  where
    ...
    buildSTreeBod = LetE ("b542",[], BoolTy, PrimAppE EqIntP [VarE "i543", LitE 0]) $
                   ...
                   (LetE ("i548",[], IntTy, PrimAppE SubP [VarE "i543", LitE 1]) $
                    ...
                    Ext $ LetLocE "l552" (AfterVariableLE "i554" "l544") $
                    Ext $ LetLocE "l553" (AfterVariableLE "b555" "l552") $
                    LetE ("x545",[],PackedTy "STree" "l553",
                             AppE "buildSTree" ["l553"] [VarE "i548"]) $
                    Ext $ LetLocE "l545" (AfterVariableLE "x545" "l553") $
...
buildSTreeMainExp = Ext $ LetRegionE (VarR "r530") $
...
sumSTreeMainExp = Ext $ LetRegionE (VarR "r771") $
                 Ext $ LetLocE "l772" (StartOfLE (VarR "r771")) $
...
sumUpMainExp :: Exp2
sumUpMainExp = Ext $ LetRegionE (VarR "r530") $
                  Ext $ LetLocE "l531" (StartOfLE (VarR "r530")) $
                  LetE ("x532",[], PackedTy "STree" "l531",
                           AppE "buildSTree" ["l531"] [LitE 2]) $
                  Ext $ LetRegionE (VarR "r536") $
  ...
  where
    ...
    setEvenFunBod = CaseE (VarE "tr570") $
      ...
      , ("Inner", [("i576","l577"),("b578","l579"),("x580","l581"),("y582","l583")],
         ...
         Ext $ LetLocE "l585" (AfterVariableLE "i576" "l584") $
         Ext $ LetLocE "l586" (AfterVariableLE "b578" "l585") $
         LetE ("x587",[],PackedTy "STree" "l586",
                  AppE "setEven" ["l581","l586"] [VarE "x580"]) $
         Ext $ LetLocE "l588" (AfterVariableLE "x587" "l586") $
  ...
  where
    ...
    sumUpSetEvenFunBod = CaseE (VarE "tr600") $
      ...
      , ("Inner", [("i607","l608"),("b609","l610"),("x611","l612"),("y613","l622")],
         ...
         Ext $ LetLocE "l615" (AfterVariableLE "i607" "l614") $
         Ext $ LetLocE "l616" (AfterVariableLE "b609" "l615") $
         ...
         Ext $ LetLocE "l620" (AfterVariableLE "x618" "l616") $
  ...
  where
    ...
    copyExprFunBod = CaseE (VarE "e700")
                     ...
                     , ("LETE", [("v706","l707"), ("rhs708", "l709"), ("bod710", "l711")],
                        ...
                        Ext $ LetLocE "l713" (AfterVariableLE "v706" "l712") $
                        LetE ("rhs714",[], PackedTy "Expr" "l713",
                                 AppE "copyExpr" ["l709","l713"] [VarE "rhs708"]) $
                        Ext $ LetLocE "l715" (AfterVariableLE "rhs714" "l713") $
    ...
    substFunBod = LetE ("old654",[],IntTy, ProjE 0 (VarE "tr653")) $
                  ...
                  , ("LETE", [("v656","l657"), ("rhs658","l659"), ("bod660", "l661")],
                     ...
                     (Ext $ LetLocE "l663" (AfterConstantLE 1 "lout653") $
                      Ext $ LetLocE "l664" (AfterVariableLE "v656" "l663") $
                      ...
                      Ext $ LetLocE "l669" (AfterVariableLE "rhs665" "l664") $
...
substMainExp :: Exp2
substMainExp = Ext $ LetRegionE (VarR "r720") $
               ...
               Ext $ LetLocE "l724" (AfterVariableLE "rhs724" "l723") $
               ...
               Ext $ LetLocE "l730" (AfterVariableLE "new730" "l729") $
...
indrBuildTreeMainExp = Ext $ LetRegionE (VarR "r800") $
...
indrRightmostMainExp = Ext $ LetRegionE (VarR "r753") $
...
indrIDSumMainExp = Ext $ LetRegionE (VarR "r806") $
                   Ext $ LetLocE "l807" (StartOfLE (VarR "r806")) $
                   LetE ("tr1",[], PackedTy "Tree" "l807",
