...
--
-- The generated loop path is intentionally narrow:
-- - it only runs when scalar-count metadata is enabled,
-- - it walks scalar-count chunk metadata for list-like inputs,
-- - it grows output chunks at input chunk boundaries, and
-- - it replaces accepted `OPT:CanVectorize` traversals outright.
--
-- This gets real loop-oriented IR into the pipeline while keeping the accepted
-- traversal shape deliberately narrow.
      ...
      case (funArgs, ins) of
        ([inEnds, outEnds, outCurs, inCurs], [CursorArrayTy arr1, CursorArrayTy arr2, CursorArrayTy arr3, CursorArrayTy arr4])
          ...
          , out == loopifiedOutTy arr1 -> do
              ...
              pure $ mkFastPathBody arr1 inEnds outEnds outCurs inCurs lliRecDCon lliBaseDCon lliScalarFieldTys plans
          | arr1 == arr2
          , arr2 == arr3
          , arr3 == arr4
          , arr1 == 1 + length lliScalarFieldTys
          , out == ProdTy [] -> do
              plans <- extractFieldPlans lliRecDCon lliScalarFieldTys funBody
              pure $ mkMutableFastPathBody inEnds outEnds outCurs inCurs lliRecDCon lliBaseDCon lliScalarFieldTys plans
        _ -> Nothing
...
mkFastPathBody :: Int -> Var -> Var -> Var -> Var -> DataCon -> DataCon -> [Ty3] -> [FieldPlan] -> Exp3
mkFastPathBody arrLen inEnds outEnds outCurs inCurs _recDCon _baseDCon scalarTys plans =
  mkLets prelude fastBody
  where
    ...
    outDconEndFinal = nm "out_dcon_end_final"
    ...
    prelude =
      ...
      , (dconInStart, [], CursorTy, indexCursorExp inCurs 0)
      , (dconOutEnd, [], CursorTy, indexCursorExp outEnds 0)
      , (dconOutLoc, [], MutCursorTy, Ext $ AddrOfCursor (VarE dconOutStart))
      , (dconInLoc, [], MutCursorTy, Ext $ AddrOfCursor (VarE dconInStart))
      , (dconOutEndLoc, [], MutCursorTy, Ext $ AddrOfCursor (VarE dconOutEnd))
      ]
      ++ concatMap mkFieldPrelude (zip3 scalarBufIndices plans [0 :: Int ..])

    mkFieldPrelude (((bufIx, _ty), FieldPlan{}, planIx)) =
      let inStart = fieldInStartVar planIx
          outStart = fieldOutStartVar planIx
          outEndStart = fieldOutEndStartVar planIx
       ...
       in [ (inStart, [], CursorTy, indexCursorExp inCurs bufIx)
          ...
          , (outEndStart, [], CursorTy, indexCursorExp outEnds bufIx)
          , (inLoc, [], MutCursorTy, Ext $ AddrOfCursor (VarE inStart))
          , (outLoc, [], MutCursorTy, Ext $ AddrOfCursor (VarE outStart))
          , (outEndLoc, [], MutCursorTy, Ext $ AddrOfCursor (VarE outEndStart))
          ]
    ...
    fastBody =
      mkLets
        ( [ (chunkLoop, [], ProdTy [], Ext $ WhileCursor countFooterLoc chunkLoopBody)
          ...
          , (outDconEndFinal, [], CursorTy, Ext $ DerefMutCursor dconOutEndLoc)
          , (inDconEnd, [], CursorTy, Ext $ DerefMutCursor dconInLoc)
          ]
          ++ concatMap mkFieldFinalLets [0 :: Int .. length plans - 1]
          ++ [ (overwriteReg, [], CursorArrayTy arrLen, Ext $ MakeCursorArray arrLen (outDconEndFinal : map fieldFinalOutEndVar [0 :: Int .. length plans - 1]))
    ...
    fieldOutEndStartVar i = fieldNm "out_end_start" i
    fieldFinalInVar i = fieldNm "in_final" i
    fieldFinalOutVar i = fieldNm "out_final" i
    fieldFinalOutEndVar i = fieldNm "out_end_final" i
    mkFieldFinalLets i =
      ...
      , (fieldFinalOutEndVar i, [], CursorTy, Ext $ DerefMutCursor (fieldOutEndLocVar i))
      ]
...
mkMutableFastPathBody :: Var -> Var -> Var -> Var -> DataCon -> DataCon -> [Ty3] -> [FieldPlan] -> Exp3
mkMutableFastPathBody inEnds outEnds outCurs inCurs _recDCon _baseDCon scalarTys plans =
  mkLets prelude fastBody
  where
    scalarBufIndices = zip [1..] scalarTys
    firstScalarIx =
