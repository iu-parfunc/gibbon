                                                ...
                                                SoAR dcReg fieldRegs -> if mode == Output
                                                                        then
                                                                          let dcreg = regionToVar dcReg
                                                                              ...
                                                                              boundsCheckDcon = [("_",[],MkTy2 IntTy, Ext $ BoundsCheck 5 dcRegArg dcLocArg)] 
                                                                              boundsCheckFields = concatMap (\(((dcon, idx), floc), freg) -> let ty = (lookupDataCon ddefs dcon) !! idx
                                                                                                                                                 size_of_ty = fromJust $ sizeOfTy (unTy2 ty)
                                                                                                                                                in [("_",[],MkTy2 IntTy, Ext $ BoundsCheck (2*(size_of_ty)) (NewL2.EndOfReg freg mode (toEndVRegVar freg)) (NewL2.Loc (LREM floc freg (toEndVRegVar freg) mode)))] 
