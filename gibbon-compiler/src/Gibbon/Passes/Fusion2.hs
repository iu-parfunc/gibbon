...
wordCount =
  map (L.head &&& L.length) . L.group . L.sort . L.words . L.map toLower

countFUS :: String -> (Int, Int)
countFUS str =
  let ls1 = splitOn "_f_" (pack str)
   in let ls2 = L.concatMap (\txt -> splitOn "_FUS" txt) ls1
       in let ls3 = L.filter (\txt -> (txt /= "") && (txt /= "_")) ls2
           in let ls4 = L.group (L.sort ls3)
               in let ls5 = L.map (\ls -> L.length ls) ls4
                   in let mx = maximum ls5
                       in (mx, L.length ls3) `debug1`
                          ((show mx) L.++ "opppa" L.++ (show ls5) L.++
                           (show ls3) L.++
                           (show str))
  ...
  where
    go exp =
      case exp of
        ...
        IfE cond thenBody elseBody -> IfE (go cond) (go thenBody) (go elseBody)
        CaseE e ls ->
          let ls' = L.map (\(x, y, exp) -> (x, y, go exp)) ls
           in CaseE e ls'
  ...
  where
    go exp mp =
      case exp of
          ...
          case bind of
            MkProdE prodList ->
              let bind' = go bind mp
                  ...
                  body' = go body mp'
               in LetE (v, ls, t, bind') body' `debug1`
                  ("here is one lol" L.++ render (pprint expin))
            otherwise ->
              let bind' = go bind mp
                  body' = go body mp
        ...
        CaseE e ls ->
          let ls' = L.map (\(x, y, exp) -> (x, y, go exp mp)) ls
           in CaseE (go e mp) ls'
        PrimAppE p ls ->
        ...
        DataConE x y ls ->
          let ls' = L.map (`go` mp) ls
           in DataConE x y ls'
        AppE v loc args -> AppE v loc (map (`go` mp) args)
        MkProdE ls ->
          let ls' = L.map (`go` mp) ls
           in MkProdE ls'
        x -> x
...
replaceLeafWithExp exp newTail = go exp
  where
    go ex =
      case ex of
...
replaceLeafWithBind exp genVar varType tailExp = go exp
  where
    go ex =
      case ex of
        L1.LetE (v, ls, t, e1) e2 -> L1.LetE (v, ls, t, e1) (go e2)
                    ...
                    V.ifoldl
                      (\tExp subscript ty ->
                         let newVar = genVar subscript
                          in L1.LetE
                               (newVar, [], ty, xDestructed V.! subscript)
                               tExp)
                      tailExp
                      (V.fromList ls2)
               in newExp
        ...
        LetE (Var symLet, _, t, bind) body ->
          let table' =
                M.insert
                  symLet
                  (DefTableEntry
                     {def = bind, fun_uses = [], all_use_count = 0, varType = t})
                  table
           ...
           in case newBody of
                ...
                Just (k, caseVars, caseExp) ->
                  ...
                  where case_subst (x1:l1) (x2:l2) exp =
                        ...
                        case_subst [] [] exp = exp
        CaseE (IfE e1 e2 e3) ls -> go $ IfE e1 (CaseE e2 ls) (CaseE e3 ls)
...
inline2 inlined_fun outer_fun = do
                 ...
                 then do
                   ...
                   let exp' =
                         addOuterTailCall
                           ...
                           (funName outer_fun)
                           (newSymbol)
                           (snd (funTy inlined_fun))
                           (L.map (\v -> (VarE v)) (tail (funArgs outer_fun)))
                   return (dataCon, vars, exp'))
            ls
...
-- or the body of the inner
  ...
  let replaceWithCall exp = do
        newVar <- gensym (toVar "innerCall")
        let rhs =
              (AppE (funName inlined_fun) [] (L.map VarE (funArgs inlined_fun)))
            body = substE oldExp (VarE newVar) exp
        return $ LetE (newVar, [], retTypeInlined, rhs) body
  newBody <-
    case (funBody outer_fun) of
...
foldFusedCallsF rule function =
  let funBody' =
        case (funBody function) of
          CaseE x ls ->
            let ls' = L.map (\(a, b, exp) -> (a, b, foldFusedCalls rule exp)) ls
             in CaseE x ls'
...
inlineConstructorConsumers fdefs exp = do
    ...
    go defTable ex =
      case ex of
        ...
        LetE (v, loc, t, lhs) bod ->
          let normal = LetE (v, loc, t, lhs) (go defTable bod)
           in case lhs of
                original@(AppE fName loc parList) ->
                  case (head parList) of
                    VarE (Var symInner) ->
                      case (getDefiningConstructor symInner defTable) of
                        Just (DataConE loc dataCons args) -> do
                          let calleeBody = funBody (fdefs M.! fName)
                              calleeArgs = funArgs (fdefs M.! fName)
                              calleeBody' =
                                replaceArgs calleeBody calleeArgs parList
                              calleeBody'' =
                                let oldExp = head parList
                                    newExp = (DataConE loc dataCons args)
                              ...
                              newTail =
                                LetE (v, [], t, leafExp) (go defTable bod)
        ...
        DataConE loc dataCons ls ->
          DataConE loc dataCons (L.map (go defTable) ls)
        _ -> ex
...
foldTupledFunctions ::
  ...
  -> PassM (Exp1)
foldTupledFunctions bodyM newFun oldCalls outputPositions syncedArgs = do
  ...
  where
    go ex newVar first =
      case ex of
                          ...
                          extractArgs fIdx (AppE _ _ (h:tail)) =
                            V.toList
                              (V.ifilter -- argIdx+1 because head is dropped (idx 0)
                                 (\argIdx arg ->
                                    not (M.member (fIdx, argIdx + 1) syncedArgs))
                                 (V.fromList tail))
        ...
        MkProdE ls -> MkProdE <$> Prelude.mapM (\x -> go x newVar first) ls
        ProjE index exp -> ProjE index <$> go exp newVar first
        CaseE e1 ls1
        --  e1' <- go e1  newVar first
         -> do
          CaseE e1 <$>
  ...
  where
    go ex dTable =
      case ex of
        ...
        IfE e1 e2 e3 -> IfE (go e1 dTable) (go e2 dTable) (go e3 dTable)
        CaseE e1 ls1 -> CaseE e1 (L.map f ls1)
          where f (dataCon, x, exp) = (dataCon, x, go exp dTable)
        AppE v loc argList -> AppE v loc (L.map (`go` dTable) argList)
...
-- add one to argIdx becasue head is deleted
                 ...
                 in if fIdx == 0
                      then ls L.++ [h] L.++ concreteArgs
                      else ls L.++ concreteArgs
                ...
                where
                  ...
                  subsVar index v ex =
                    let oldExp = VarE (fst v)
                        newExp =
                          (VarE
                             (toVar (L.map toLower (dataCons L.++ show index))))
                     in substE oldExp newExp ex
  let traversedTreeDDef =
        lookupDDef
          ddefs
          (case traversedType of
             (PackedTy tName _) -> tName)
...
-- stores the result
                  ...
                  ProdTy ls2 ->
                    let newElements =
                          V.toList
                            (V.imap
                               (\subscript _ ->
                                  VarE (createOutVar index subscript))
                               (V.fromList ls2))
                     in (ls L.++ newElements)
                  _ ->
  ...
  let extendedCase =
        L.foldr addConstructorBody topLevelExpr (dataCons traversedTreeDDef)
        where
  ...
  where
    go ex tb =
      case ex of
          ...
          let tb' =
                case rhs of
                  callExp@(AppE fName _ argList@((VarE inputTree):tail)) ->
                    let otherCalls =
                          if M.member inputTree tb
                            then (tb M.! inputTree)
                            else []
                     in if (isTupleable (fDefs M.! fName)) &&
                           (haveIndependentArgsNew tail otherCalls)
                          then let addCall Nothing =
                                     Just [(boundedVar, callExp)]
                                   addCall (Just ls) =
                                     Just $ L.nub ((boundedVar, callExp) : ls)
        ...
        DataConE loc datacons ls -> L.foldl f tb ls
          where f table exp = go exp table
      ...
      ---add a check that there is no nested case (maybe)
        ...
        collectDependentVarsExp exp =
          case exp of
            ...
            IfE cond thenBody elseBody ->
              S.unions
                [ collectDependentVarsExp cond
                , collectDependentVarsExp thenBody
                , collectDependentVarsExp elseBody
                ]
            DataConE _ _ expList ->
              S.unions (L.map collectDependentVarsExp expList)
            ProjE index exp -> collectDependentVarsExp exp
            LitE _ -> S.empty
            LetE (Var s, loc, t, rhs) body ->
              S.unions
                [collectDependentVarsExp rhs, collectDependentVarsExp body]
            x -> error ("please handle me explicitly" L.++ (show x))
...
tuple_entry ::
     DDefs Ty1 -> FunDefs1 -> Exp1 -> [Var] -> Int -> PassM (Exp1, FunDefs1)
...
tuple ddefs fdefs oldExp_ argsVars depth = do
  ...
  -- same input
      let candidates1 =
            L.filter
              f
              (M.toList (buildTupleCandidatesTable fdefs oldExp argsVars))
         --    `debug` ("looking for candidates for " L.++ render (pprint oldExp))
            where
              f (_, ls) = L.length ls > 1
...
-- arg 2 in f1
            ...
            L.map
              (\(traversedVar, ls) ->
                 let sortedCalls = L.sortOn f ls
                       where
                         f exp@(AppE fName _ _) = (fName, exp)
                     syncArgsLocs = computeSyncedArgs sortedCalls --`debug` ("done1")
                  in ( constructName sortedCalls (M.toList syncArgsLocs)
                     , sortedCalls
    ...
    --(newExp, fdefs') <-  Control.Monad.foldM go (oldExp, fdefs)  candidates2
      if L.length candidates2 > 0
        ...
        else return (oldExp, fdefs) `debug` (show "no candidates")
  where
      ...
      case M.lookup tupledFName fdefs of
        ...
        Nothing -> do
          let functionsToTuple = L.map getCalledFunDef callExpressions
                where
                  getCalledFunDef callExpr =
                    case callExpr of
                      (AppE fName _ _) ->
                        case M.lookup fName fdefs of
                          Just fdef -> fdef
          tupledFunction_ <-
            tupleListOfFunctions ddefs functionsToTuple tupledFName syncArgsLocs `debug`
            ("funcs to tuple" L.++ (show functionsToTuple))
          tupledFunction <- freshFunction tupledFunction_
          let tupledFunction' =
                tupledFunction {funBody = cleanExp (funBody tupledFunction)} `debug`
                ("tupled f is :" L.++ (render (pprint tupledFunction_)))
          ...
          (recTupledBody, newDefs) <-
            tuple_entry
              ...
              traversedArg
              (depth + 1) `debug`
            ("\n done tupling :" L.++ (show tupledFName) L.++
             (render (pprint tupledFunction')))
          ...
          exp' <-
            foldTupledFunctions
              ...
              (getOutputStartPositions fdefs'' callExpressions)
              syncArgsLocs `debug`
            ("fold2 before folding" L.++ render (pprint exp))
          let exp'' = simplifyProjections exp'
    ...
    constructName ls syncArgsLocs =
      let syncedArgsText = L.foldr f "" syncArgsLocs
            where
              f ((f1, id1), (f2, id2)) txt =
                txt L.++ "f" L.++ (show f1) L.++ "_" L.++ (show id1) L.++
                "_from_" L.++
                "f" L.++
                show (f2) L.++
                "_" L.++
                ...
                "_n_"
       in toVar
            ("_TUP_" L.++ L.foldl appendName "" ls L.++ syncedArgsText L.++
             "_TUP_")
              ...
              f (funPos, argPos, argExp) (firstAppear, redundant) =
                if M.member argExp firstAppear
                  then ( firstAppear
                       , M.insert
                           (funPos, argPos)
                           (firstAppear M.! argExp)
                           redundant)
                  else (M.insert argExp (funPos, argPos) firstAppear, redundant)
...
fixCalls ::
     Exp1 -> FunDef1 -> FunDef1 -> M.Map Int Int -> M.Map Int Int -> Var -> Exp1
          ...
          case rhs of
            AppE v ls args ->
              if v == funName fdefOld
                then let t' = snd (funTy fdefNew)
                      in let rhs' = AppE newName ls args
                          in let body' =
                                   L.foldl
                                     (\ex (i, j) ->
                                        let oldExp = ProjE i (VarE (Var y))
                                            newExp = getExpAtIndex args j
                                         in substE oldExp newExp ex `debug`
                                            ("replacing1" L.++ (show oldExp) L.++
                                             "with" L.++
                                             (show newExp) L.++
                                             (show outputFromInput)))
                                     body
                                     (M.toList outputFromInput)
                                 body'' =
                                   L.foldl
                                     (\ex (i, j) ->
                                        if i == j
                                          then ex
                                                ...
                                                in substE oldExp newExp ex `debug`
                                                   ("replacing2" L.++
                                                    ...
                                                    (show newExp)))
                                     body'
                                     (M.toList redirectMap) -- in
                              in LetE (Var y, loc, t', rhs') (go body'')
                else LetE (Var y, loc, t, rhs) (go body)
            _ -> LetE (Var y, loc, t, rhs) (go body)
        otherwise -> otherwise
...
getOutputStartPositions fdefs callExpressions =
  let functionsArgsLengths = L.map getCalledFunDef callExpressions
   in let ls =
            L.foldl (\ls i -> ls L.++ [i + L.last ls]) [0] functionsArgsLengths
       in V.fromList ls
  where
    getCalledFunDef callExpr =
      case callExpr of
...
fuse ddefs fdefs innerVar outerVar prevFusedFuncs = do
  ...
  newName <-
    if verbosity config >= 0
      then return
             (toVar
                ("_FUS_f_" ++
                 fromVar outerVar ++ "_f_" ++ fromVar innerVar ++ "_FUS_"))
      else gensym "_FUSE_"
  ...
  let step5 =
        step4 {funBody = removeUnusedDefsExp (funBody step4)} `debug`
        ("newName is :" L.++ (show newName) L.++ render (pprint step4))
              ...
              p3 =
                case ((funBody innerDef)) of
                  CaseE _ ls ->
                    not
                      (L.foldr
                         (\(_, _, exp) res -> res && (hasConstructorTail exp))
                         True
                         ls)
                  _ -> True
              p4 =
...
tuple_pass ddefs fdefs = foldM tupleFunction fdefs fdefs
  where
    tupleFunction defs' f = do
      let fName           = funName f
      if L.isPrefixOf "_FUS" (fromVar fName) ||
         L.isPrefixOf "_TUP" (fromVar fName)
        then do
          (tupledBody, tupleDefs) <-
            tuple_entry ddefs fdefs (funBody f) (funArgs f) 0 `debug`
            ("run tuple for" L.++ (show (funName f)))
  ...
  where
    go body processed fdefs prevFusedFuncs = do
      ...
      case potential of
        ...
        Just ((inner, outer), outerDefVarSymbol) -> do
          if violateRestrictions fdefs inner outer depth
            then go body ((inner, outer) : processed) fdefs prevFusedFuncs
...
--  render (pprint ( fusedDefs M.! fNew  ))))
              ...
              (retFuncBody, retFunDefs, retFusedFunctions) <-
                fuse_pass
                  ddefs
                  fusedDefs
                  (FusePassParams
                     (funBody fusedFunction)
                     (funArgs fusedFunction)
                     newFusedFunctions
                     newProcessed
                     (depth + 1))
              ...
              let foldedBody =
                    foldFusedCalls_Entry (outer, inner, -1, fNew) body
              if validFused
                then let body' = removeUnusedDefsExp foldedBody
                      in go (body') newProcessed newDefs newFusedFunctions2
                else go body newProcessed fdefs prevFusedFuncs


--`debug` ("new fused:" L.++ (
--clean
tupleAndOptimize ::
     DDefs Ty1 -> FunDefs1 -> Exp1 -> Bool -> Int -> PassM (Exp1, FunDefs1)
tupleAndOptimize ddefs fdefs mainExp firstTime depth = do
  newDefs <- tuple_pass ddefs fdefs
  if depth > 0 || (not firstTime && (newDefs == fdefs))
    then return (mainExp, newDefs)
            --return newDefs
    else let (mainExp', fdefs') =
               (redundancy_output_pass newDefs mainExp firstTime 0)
          in tupleAndOptimize ddefs fdefs' mainExp' False (depth + 1) `debug`
             "run new tuple round"
  ...
  (main', funs') <-
    case main of
      ...
      Just (mainBody, ty) -> do
        ...
        --        ) newDefs
        (mainBody'', newDefs') <-
          tupleAndOptimize defs (M.union funs newDefs) mainBody' True 0
        let newDefs'' =
              M.filter
                (\f ->
                   case snd (funTy f) of
...
redundancy_output_pass fdefs mainExp firstTime depth =
  ...
  where
    pass2F fdefs fName (redirectMap, outPutFromInput, newName) =
      M.map
        (pass2Fsub fdefs fName (redirectMap, outPutFromInput, newName))
        fdefs
    ...
    pass1F orgFdefs (fdefs, rules) f =
      let fName = funName f
       in if L.isPrefixOf "_TUP" (fromVar fName)
            then let testedPositions = testAllOutputPositions orgFdefs fName
                     newName = toVar (fromVar fName L.++ "outputFixed")
                  in if M.member newName fdefs
                       then (fdefs, rules)
                       else let (fNew, redirectMap, outPutFromInput) =
                             ...
                             in ( M.insert (funName fNew) fNew fdefs
                                , M.insert
                                    fName
                                    (redirectMap, outPutFromInput, funName fNew)
                                    rules)
...
testAllOutputPositions fdefs fName =
  ...
  where
    ...
    loop1 i j n testedPositions =
      if j >= n
        then testedPositions
        else if i >= n
               then loop1 0 (j + 1) n testedPositions
               else loop1
                      (i + 1)
                      j
                      n
                      (snd
                         (testTwoOutputPositions
                            fdefs
                            (fName, i, j)
                            testedPositions))
...
testTwoOutputPositions fdefs (fName, i, j) testedPositions =
  if i == j
    then (True, testedPositions)
    else case M.lookup (fName, i, j) testedPositions of
             ...
             case M.lookup (fName, j, i) testedPositions of
               ...
               Nothing ->
                 let (cond, inductiveAssumption, unresolvedConditions) =
                  ...
                  in if cond
                       then let (cond', inductiveAssumption') =
                        ...
                        --  are done and correct !
                            ...
                            , M.insert (fName, i, j) False testedPositions)
  where
    ...
    -- call recursively.
     =
      let (res, inductiveAssumption', unresolvedConditions') =
            S.foldl
              foo
              (True, inductiveAssumption, S.empty)
              unresolvedConditions
          unresolvedConditions'' =
            S.filter
              (\(f, i, j) ->
       ...
       in if res && S.null unresolvedConditions''
            then (res, inductiveAssumption')
                   ...
                   then (False, S.empty)
                   else testTwoOutputPositionsRec
                          inductiveAssumption'
                          unresolvedConditions''
    foo ::
    ...
    foo (condInput, assumptionsInput, unresolvedInput) (fName, i, j) =
      let (cond, inductiveAssumption, unresolvedConditions) =
            extractAssumptionAndConditions fName i j
       in ( cond && condInput
          , S.union assumptionsInput inductiveAssumption
          , S.union unresolvedConditions unresolvedInput)
    extractAssumptionAndConditions ::
         Var
      -> Int
      -> Int
      -> (Bool, S.Set (Var, Int, Int), S.Set (Var, Int, Int))
...
--`debug` (show (fName) L.++ "inlined body\n"L.++ (render (pprint exp)))
       in case inlinedContent of
            CaseE e ls ->
              let parametrizedExprsList = L.map parametrizeProdExprs ls
                  cond =
                    L.foldl (checkExpressions i j) True parametrizedExprsList
               in if cond
                    then let inductiveAssumption =
                               S.insert (fName, i, j) S.empty
                             unresolvedConditions =
                               L.foldl
                                 (collectConditions i j)
                                 S.empty
                    ...
                    else (False, S.empty, S.empty)
    parametrizeProdExprs ::
         (DataCon, [(Var, ())], Exp1) -> [(Exp1, [(Int, Var, Var)])]
    ...
    parsCheck [] []                             = True
    parsCheck ((_, v1, _):ls1) ((_, v2, _):ls2) = v1 == v2 && parsCheck ls1 ls2
    ...
    collectConditionsPars ((idx1, v1, f):ls1) ((idx2, v2, _):ls2) =
      let sNext = collectConditionsPars ls1 ls2
       in if idx1 == idx2
            then sNext
            else S.insert (f, idx1, idx2) sNext
...
collectVars = recur
  where
    recur ex =
      case ex of
        LetE (v, ls, t, AppE {}) body -> S.insert v (recur body)
        MkProdE {}                    -> S.empty
...
collectVarToFuncs = recur
  where
    recur ex =
      case ex of
        LetE (v, ls, t, (AppE f _ _)) body -> M.insert v f (recur body)
        MkProdE {}                         -> M.empty
...
parametrizeExp ::
     S.Set Var -> M.Map Var Var -> Exp1 -> (Exp1, [(Int, Var, Var)])
    ...
    recur ex ls =
      ...
      -- TODO: this is work around (correct not complete)[should be also handled]
       ...
       -- error( "CaseE not expected in parametrizeExp" ++ (render (pprint ex )))
        ...
        x@(ProjE i ((VarE v))) ->
          if S.member v vars
            then let exp' = VarE (toVar ("par" L.++ show (L.length ls)))
                     ls' = ls L.++ [(i, v)]
                  in (exp', ls')
            else (x, ls)
...
removeRedundantOutput ::
     FunDef1
  -> M.Map (Var, Int, Int) Bool
  -> (FunDef1, M.Map Int Int, M.Map Int Int)
removeRedundantOutput fdef testedPositions =
  let outputsFromInputs = getOutputsFromInput fdef
   in let outputTuples = V.fromList (collectOutputs (funBody fdef))
       in let firstTuple = outputTuples V.! 0 --return vector
           in let loop i j =
                  ...
                  removedPositions =
                    S.toList
                      (S.union
                         ...
                         (S.fromList
                            (L.map (\(x, y) -> x) (M.toList outputsFromInputs))))
                  newFunType =
                    let oldOutTypesList =
                          V.fromList
                            (case snd (funTy fdef) of
                    ...
                    V.ifoldl
                      (\mp idx (i, ls) ->
                         let mp' = M.insert i idx mp
                          ...
                          in mp'')
                      M.empty
                      (V.fromList (M.toList finalMap))
                  removedUnhandled =
                    L.map (\(a, b) -> a) (M.toList outputsFromInputs)
                  redirectMap' =
                    L.foldl
                      (\mp i -> M.delete i mp)
                      redirectMap
                      ...
                      { funName =
                          if (newFunBody == funBody fdef)
                            then funName fdef'
                                   ...
                                   (fromVar (funName fdef') L.++ "outputFixed")
                      }
...
eliminateInputArgs fdefs fNameOld syncedArgs =
  let newName =
        toVar
          ((M.foldlWithKey
              buildName
              ((fromVar fNameOld) L.++ "elimpass_")
              syncedArgs) L.++
           "_elimpass")
      fdefs' =
        if M.member newName fdefs
          then fdefs
          else let oldFdef = fdefs M.! fNameOld
                   ...
                   newBody =
                     M.foldlWithKey
                       (\exp k v ->
                          if (v < 1000)
                            then let oldExp = VarE ((V.fromList oldArgs) V.! k)
...
getOutputsFromInput func =
  let body = funBody func
      leafProducts =
        case body of
          CaseE e ls -> L.map (\(_, _, exp) -> getLeafProdExpressions exp) ls
      inputVars = funArgs func
      candidatesList =
        L.map
          (\exprList -> V.ifoldl f M.empty (V.fromList exprList))
          leafProducts
        where
          f out idx outExp =
            case outExp of
              ...
              otherwise -> out
      candidatesListSets =
        L.map (\mp -> S.fromList (M.toList mp)) candidatesList
      intersectionsSet =
        L.foldl S.intersection (L.head candidatesListSets) candidatesListSets
          ...
          (fdefs'', ls') = L.foldl f (fdefs', []) ls
            where
              f (fdefsInner, lsInner) (dataCon, vars, exp) =
                let (fdefsInner', exp') =
                      removeRedundantInputExp fdefsInner exp mode
                 ...
                 then let redundantPositions =
                            if (mode)
                              then snd
                                     ...
                                     (findRedundantPos_UnusedArgs fName)
                                     M.empty
                                     (V.fromList args)
                       ...
                       in if M.null redundantPositions
                            then boringCase
                            else let (fNameNew, fdefsNew) =
                                       eliminateInputArgs
                                         fdefs'
                                         fName
                                         redundantPositions
                            ...
                            newCall =
                              AppE
                                ...
                                (V.toList
                                   (V.ifilter
                                      (\idx _ ->
                                         M.notMember idx redundantPositions)
                                      (V.fromList args)))
                         in (fdefsNew, newCall)
        else (fdefs, x)
  ...
  where
    findRedundantPos (firstAppear, redundant) argIdx arg =
      if M.member arg firstAppear
        then (firstAppear, M.insert argIdx (firstAppear M.! arg) redundant)
        else (M.insert arg argIdx firstAppear, redundant)
    findRedundantPos_UnusedArgs fName mp argIdx arg =
      let callee = fdefs M.! fName
       in if (isUsedArg
                (funBody callee)
                ((V.fromList (funArgs callee)) V.! argIdx))
            then mp
            else M.insert argIdx 100000 mp
    isUsedArg exp var =
      case exp of
        ProjE i e -> isUsedArg e var
        VarE v' -> v' == var
        CaseE e ls ->
          let b1 = isUsedArg e var
              b2 =
                L.foldl
                  (\res (dataCon, vars, ex) -> (res || isUsedArg ex var))
                  False
                  ls
           in b1 || b2
        AppE fName loc args ->
...
removeRedundantInputFunc fdefs fdef =
  let (fdefs', exp) = removeRedundantInputExp fdefs (funBody fdef) True
      ...
      (fdefs'', exp') =
        removeRedundantInputExp (M.insert (funName fdef) fdef' fdefs') exp False
  ...
  let fdefs' =
   ...
   in let (fdefs'', mainExp') = removeRedundantInputsMainExp fdefs' mainExp
       in if (fdefs'' == fdefs && mainExp' == mainExp) || depth > 5
            ...
            else redundancy_input_pass_rec fdefs'' mainExp' (depth + 1) `debug`
                 ("repeeat")
