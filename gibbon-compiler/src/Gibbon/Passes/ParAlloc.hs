...
parAllocExp ddefs fundefs env2 reg_env after_env mb_parent_id pending_binds spawned boundlocs region_on_spawn ex =
  case ex of
    ...
    Ext ext  ->
      case ext of
        LetRegionE r sz bod       -> Ext <$> (LetRegionE r sz) <$>
        ...
        LetParRegionE r sz bod    -> Ext <$> (LetParRegionE r sz) <$>
                                    parAllocExp ddefs fundefs env2 reg_env after_env mb_parent_id pending_binds spawned (S.insert (regionToVar r) boundlocs) region_on_spawn bod
              ...
              then
                pure $ LetE (cont_id, [], IntTy, Ext GetCilkWorkerNum) $
                       ...
                       IfE (VarE not_stolen)
                           ...
                           (Ext $ LetParRegionE newreg Undefined $ Ext $ LetLocE newloc (StartOfLE newreg) bod1)
              else
                pure $ Ext $ LetParRegionE newreg Undefined $ Ext $ LetLocE newloc (StartOfLE newreg) bod1
