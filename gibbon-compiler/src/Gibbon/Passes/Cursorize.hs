...
handleIndexingSoACursors :: Maybe (Var, LocArg) -> MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> (LocVar, Var) -> (Int, Int) -> LocVar -> M.Map FreeVarsTy Var -> PassM (Var, M.Map FreeVarsTy Var, [(Var, [()], Ty3, Exp3)], MutableLocPtsToEnv, MutableLocOldValueEnv)
handleIndexingSoACursors mbvarlarg m1 m2 forceMutable (arrLoc, arrName) (start, end) locvar var_env = do
                                                                    ...
                                                                    True -> do 
                                                                            ...
                                                                            case mbvarlarg of 
                                                                                 Just (aliasvar, aliaslocarg) -> do 
                                                                                                                  ...
                                                                                                                  let m1' = updateMutableLocPtsToEnv arrLoc m1 (arrName, Just arrLoc, Nothing, S.fromList [arrLocNameInsideCase, arrLocNameInsideCase]) True
                                                                                                                  ...
                                                                                                                  let deref_var = case deref_bnd of 
                                                                                                                                      [(v, _, _, _)] -> v
                                                                                                                                      [] -> arrName
                                                                                                                  return (deref_var, var_env, [(arrName, [], MutCursorTy, Ext $ AddrOfCursor $ Ext $ IndexCursorArray par_var start)] ++ deref_bnd, m1', m2')
                                                                                 Nothing -> do
                                                                                              let m1' = updateMutableLocPtsToEnv arrLoc m1 (arrName, Just arrLoc, Nothing, S.empty) True
                                                                                              (m2', deref_bnd) <- updateMutableLocOldValueEnv arrLoc m2 (arrName, Just arrLoc, Nothing, S.empty) True
                                                                                              let deref_var = case deref_bnd of 
                                                                                                              [(v, _, _, _)] -> v
                                                                                                              [] -> arrName
                                                                                              return (deref_var, var_env, [(arrName, [], MutCursorTy, Ext $ AddrOfCursor $ Ext $ IndexCursorArray par_var start)] ++ deref_bnd, m1', m2')
                                                                    False -> return (arrName, var_env, [(arrName, [], CursorTy, Ext $ IndexCursorArray par_var start)], m1, m2)
                                                SoA{} -> do
                                                         ...
                                                         return (arrName, var_env, bnds ++ make_cur_arr_let, m1, m2)
...
cursorizeFunDef ddefs fundefs FunDef {funName, funTy, funArgs, funBody, funMeta} = do
  ...
  let isFunTailRec = case funRec of 
                            TailRec -> True 
                            _ -> False
  ...
  let inLocs = inLocVars funTy
      ...
      funTy' = cursorizeArrowTy needOptTailCalls funTy
  ...
  let (inCurBinds, m1, m2) = case inLocA of
                      ...
                      let loc = toLocVar loca
                          ...
                          m11' = case (isMutModality' modality) of
                                            True -> let varNameToUseForLoc = case proj of 
                                                                                  VarE v -> v
                                                                                  _ -> var_for_loc 
                                                      in M.insert loc (varNameToUseForLoc, Just loc, Just reg, S.insert var_for_loc S.empty) m11
                                            False -> m11
      ...
      initTyEnv =
        M.fromList $
          (map (\(a, b) -> (a, MkTy2 (cursorizeInTy needOptTailCalls Nothing (unTy2 b)))) $ zip funArgs in_tys)
            ++ ( concatMap
                   ( \(LRM l r m) ->
  ...
  bod <-
    if hasPacked (unTy2 out_ty)
      then
        do 
          (funBody', _, _, _) <-  cursorizePackedExp m1 m2 needOptTailCalls freeVarToVarEnv' initTyEnvl ddefs fundefs M.empty initTyEnv M.empty funBody
          return $ fromDi funBody'
      else do 
        (funBody', _, _, _) <- cursorizeExp m1 m2 needOptTailCalls freeVarToVarEnv' initTyEnvl ddefs fundefs M.empty initTyEnv M.empty funBody
        return funBody'
  ...
  where
    ...
    cursorizeInTy optTail modality ty =
      case ty of
        IntTy -> IntTy
        CharTy -> CharTy
        FloatTy -> FloatTy
    ...
    cursorizeArrowTy optTailRec ty@ArrowTy2 {arrIns, arrOut, locVars, locRets} =
      let -- Regions corresponding to ouput cursors. (See [Threading regions])
          ...
          ret_curs =
            L.map
              ( \lret -> case lret of
                  EndOf (LRM l _ m) -> getCursorizeTyFromLocVar'' (Just m) optTailRec l
              )
              locRets
          ...
          newOut =
            mapPacked
              (\var loc -> case optTailRec of 
                                  False -> ProdTy [getCursorizeTyFromLocVar'' Nothing optTailRec loc, getCursorizeTyFromLocVar'' Nothing optTailRec loc]
          ...
          outCurTys =
            map
              ( \(LRM l _ m) -> getCursorizeTyFromLocVar'' (Just m) optTailRec l
              )
              outCurs
          inRegs =
            map
          ...
          newIns = map (cursorizeInTy optTailRec Nothing) in_tys
            --if useSoA
...
cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex =
  case ex of
    ...
    AppE {} -> cursorizeAppE m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex
    ...
    PrimAppE pr args -> do 
                         res <- mapM (go m1 m2 freeVarToVarEnv) args
    ...
    LetE (v, _locs, _ty, (PrimAppE (ReadPackedFile path tyc reg ty2) [])) bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeReadPackedFile m1 m2 optTailCall freeVarToVarEnv' lenv ddfs fundefs denv tenv senv True v path tyc reg ty2 bod
    ...
    LetE (_v, _locs, _ty, (MkProdE _ls)) _bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_prod, freeVarToVarEnv'', m1', m2') <- cursorizeProd m1 m2 optTailCall freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
    ...
    LetE (_v, _locs, ty, ProjE {}) _bod | isPackedTy (unTy2 ty) -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeProj m1 m2 optTailCall freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
      return $ (ret_e, M.union freeVarToVarEnv' freeVarToVarEnv'', m1', m2')
    LetE (_v, _locs, _ty, SpawnE {}) _bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeSpawn m1 m2 optTailCall freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
      return (ret_e, M.union freeVarToVarEnv' freeVarToVarEnv'', m1', m2')
    LetE (_v, _locs, _ty, SyncE) _bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeSync m1 m2 optTailCall freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv ex
      return $ (ret_e, M.union freeVarToVarEnv' freeVarToVarEnv'', m1', m2')
    LetE (v, _locs, ty, rhs@(Ext (SSPush _ start _ _))) bod -> do
      case M.lookup (unwrapLocVar start) tenv of
        Nothing -> go m1 m2 freeVarToVarEnv bod
        Just {} -> do
          (rhs', fenv1, m1', m2') <- go m1 m2 freeVarToVarEnv rhs
          (bod', fenv2, m1'', m2'') <- go m1' m2' fenv1 bod
          let ty' = cursorizeTy m1'' m2'' optTailCall Nothing (unTy2 ty)
    ...
    LetE bnd@(_, _locs, _, _) bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (ret_e, freeVarToVarEnv'', m1', m2') <- cursorizeLet m1 m2 optTailCall freeVarToVarEnv' lenv False ddfs fundefs denv tenv senv bnd bod
      return $ (ret_e, M.unions [freeVarToVarEnv, freeVarToVarEnv', freeVarToVarEnv''], m1', m2')
    IfE a b c -> do 
                  (a', e1, m1', m2') <- go m1 m2 freeVarToVarEnv a
                  (b', e2, m1'', m2'') <- go m1' m2' freeVarToVarEnv b
                  (c', e3, m1''', m2''') <- go m1'' m2'' freeVarToVarEnv c
                  return (IfE a' b' c', M.unions [freeVarToVarEnv, e1, e2, e3], m1''', m2''')
    MkProdE ls -> do 
      res <- mapM (go m1 m2 freeVarToVarEnv) ls
      let ls' = map fst4 res
    ...
    ProjE i e -> do 
                  (e', env, m1', m2') <- go m1 m2 freeVarToVarEnv e
    ...
    WithArenaE v e -> do
      (e', env, m1', m2') <- cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv (M.insert v (MkTy2 ArenaTy) tenv) senv e
    ...
    Ext ext ->
      case ext of
        ...
        LetLocE locarg rhs bod -> do
          ...
          let ty2_of_loc = getCursorizeTyFromLocVar'' (getModality locarg) False loc
          let ty3_of_loc = getCursorizeTyFromLocVar (getModality locarg) False loc
          freeVarToVarEnv' <- do
            case loc of
              Single l ->
          ...
          let locs_variable = case (M.lookup (fromLocVarToFreeVarsTy loc) freeVarToVarEnv') of
                ...
                Nothing -> case loc of
                  ...
                  SoA _ _ -> error "cursorizeExp: LetLocE: unexpected location variable"
          (rhs_either, m1', m2') <- cursorizeLocExp m1 m2 optTailCall freeVarToVarEnv' denv tenv senv locarg rhs
              ...
              case rhs of
                FromEndLE {} ->
                  if isBound locs_var tenv
                    then cursorizeExp m1' m2' optTailCall freeVarToVarEnv' lenv ddfs fundefs denv (M.insert locs_var (MkTy2 ty2_of_loc) tenv''') senv' bod
        ...
        LetRegionE reg sz endmut _ bod -> do
          ...
          let reg_ty = getCursorizeTyFromRegVar' Nothing optTailCall reg_var
          ...
          (bod, freeVarToVarEnv'''', m1', m2') <- cursorizeExp m1 m2 optTailCall freeVarToVarEnv''' lenv ddfs fundefs denv tenv'' senv bod
          ...
          (cur_loc, cur_var, additional_bnds', m1', m2') <- case fromJust cur_modality of 
                                                            L2.OutputMutable -> if M.member loc_seen m2 
                                                                             ...
                                                                             then do
                                                                               ...
                                                                               let m11 = M.insert (loc_seen) (deref_var, Nothing, Just bound_reg, S.empty) m1
                  ...
                  (additional_bnds, bound_var', im1', im2') <- do
                    case bound_var_ty of
                      Just (MkTy2 MutCursorTy) -> do
                        dereference_bound_var <- gensym "deref"
                        let bnd = [(dereference_bound_var, [], CursorTy, Ext $ DerefMutCursor bound_var)]
                        let im11 = M.insert bound_loc (dereference_bound_var, Just bound_loc, Just bound_reg, S.empty) im1
                  ...
                  (additional_bnds', cur_var', im1'', im2'') <- do
                    case cur_var_ty of
                      Just (MkTy2 MutCursorTy) -> do
                        ...
                        let im11 = M.insert cur_loc (dereference_cur_var, Just cur_loc, Just bound_reg, S.empty) im1'
        ...
        LetAvail vs bod -> do
                            (bod', env, m1', m2') <- go m1 m2 freeVarToVarEnv bod
        ...
        LetRegE loca rhs bod -> do
          ...
          let ty_of_loc = getCursorizeTyFromRegVar'' modality optTailCall loc
          let ty2_of_loc :: Ty2 = getCursorizeTyFromRegVar' modality optTailCall loc
          freeVarToVarEnv' <- do
            case loc of
              SingleR l ->
          ...
          case rhs_either of
            Right (rhs', bnds', tenv'', senv') -> do
              ...
              case rhs of
                ...
                _ -> case ty_of_loc of
                  MutCursorTy -> do
                       (bod', env, m1'', m2'') <- cursorizeExp m1' m2' optTailCall freeVarToVarEnv' lenv ddfs fundefs denv (M.insert locs_var (ty2_of_loc) tenv''') senv' bod
            ...
            Left denv' -> do
                          (bod', env, m1'', m2'') <- cursorizeExp m1' m2' optTailCall freeVarToVarEnv' lenv ddfs fundefs denv' tenv' senv bod
  ...
  where
    go gm1 gm2 fenv = cursorizeExp gm1 gm2 optTailCall fenv lenv ddfs fundefs denv tenv senv
...
cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex =
  case ex of
    ...
    VarE v -> do
      ...
      case (unTy2 ty) of
        PackedTy _ l -> -- For now we just return void, in case the function is tailRec
    ...
    AppE {} -> do
               (ex', freeVarToVarEnv', m1', m2') <- cursorizeAppE m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex
    ...
    LetE (v, _locs, _ty, (PrimAppE (ReadPackedFile path tyc reg ty2) [])) bod -> do
      (bod', freeVarToVarEnv', m1', m2') <- cursorizeReadPackedFile m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv True v path tyc reg ty2 bod
      return (Di bod', freeVarToVarEnv', m1', m2') 
    LetE (v, _locs, _ty, (PrimAppE (DictLookupP (MkTy2 (PackedTy _ ploc))) vs)) bod ->
      do
        vs' <- forM vs $ \w -> cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv w
        ...
        (bod', freeVarToVarEnv', m1', m2') <- go m1 m2 freeVarToVarEnv tenv' senv bod
        return (onDi bnd bod', M.unions $ [freeVarToVarEnv'] ++ envs', M.unions (m1s' ++ [m1]), M.unions (m2s' ++ [m2]))

    LetE (_v, _locs, _ty, (MkProdE _ls)) _bod -> do
      (ex', freeVarToVarEnv', m1', m2') <- cursorizeProd m1 m2 optTailCall freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
      return (dl ex', freeVarToVarEnv', m1', m2') 
    LetE (_v, _locs, ty, ProjE {}) _bod
      | isPackedTy (unTy2 ty) -> do
          (ex', freeVarToVarEnv', m1', m2') <- cursorizeProj m1 m2 optTailCall freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
          return (dl ex', freeVarToVarEnv', m1', m2') 
    MkProdE ls -> do
        ...
        forM (zip tys ls) $ \(ty, e) -> do
          case ty of
            _ | isPackedTy (unTy2 ty) -> do 
                                        (e', freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv e
                                        return (fromDi e', freeVarToVarEnv', m1', m2') 
            _ -> cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv e
      let es = map fst4 res
      let envs = map snd4 res
      let m1s' = map thd4 res
      let m2s' = map fth4 res
    ...
    ProjE i e -> do 
                 (e', env, m1', m2') <- go m1 m2 freeVarToVarEnv tenv senv e 
                 return (dl $ ProjE i (fromDi e'), env, m1', m2') 
    LetE (_v, _locs, _ty, SpawnE {}) _bod -> do 
      (ex', freeVarToVarEnv', m1', m2') <- cursorizeSpawn m1 m2 optTailCall freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
      return (dl ex', freeVarToVarEnv', m1', m2') 
    LetE (_v, _locs, _ty, SyncE) _bod -> do
      (ex', freeVarToVarEnv', m1', m2') <- cursorizeSync m1 m2 optTailCall freeVarToVarEnv lenv True ddfs fundefs denv tenv senv ex
      return (dl ex', freeVarToVarEnv', m1', m2') 
    LetE (v, _locs, ty, rhs@(Ext (SSPush _ start _ _))) bod ->
      case M.lookup (unwrapLocVar start) tenv of
        Nothing -> go m1 m2 freeVarToVarEnv tenv senv bod
        Just {} -> do
          (rhs', env1, m1', m2') <- go m1 m2 freeVarToVarEnv tenv senv rhs
          let ty' = cursorizeTy m1' m2' optTailCall Nothing (unTy2 ty)
          (bod', env2, m1'', m2'') <- go m1' m2' freeVarToVarEnv (M.insert v ty tenv) senv bod
    ...
    LetE bnd@(_, _locs, _, _) bod -> do
      freeVarToVarEnv' <-
        foldrM
          ...
          freeVarToVarEnv
          _locs
      (bod', freeVarToVarEnv'', m1', m2') <- cursorizeLet m1 m2 optTailCall freeVarToVarEnv' lenv True ddfs fundefs denv tenv senv bnd bod    
    ...
    IfE a b c -> do
      (Di b', env1, m1', m2') <- go m1 m2 freeVarToVarEnv tenv senv b
      (Di c', env2, m1'', m2'') <- go m1 m2 freeVarToVarEnv tenv senv c
      (a', env3, m1''', m2''') <- cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv a
      ...
      if (not (isSoALoc (toLocVar slocarg)))
        then do
          let sloc_loc = toLocVar slocarg
              ...
              (sloc_loc_to_write, sloc) = if isMutModality $ fromJust modality_sloc
                              ...
                              then 
                                case M.lookup sloc_loc m2 of
                                          ...
                                          Nothing -> error "expected to have old value for the output mutable location!"
                                          Just (oldv, oldl, _, _) -> case oldl of 
                                                                    Nothing -> (Nothing, oldv)
                                                                    Just ol -> (Just ol, oldv)
                              else (Just sloc_loc, getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy sloc_loc)) 
          ...
          let sloc_loc = toLocVar slocarg
              ...
              go2 marker_added fvarenv aft_dloc from_rec_end aft_flocs [] = do
                ...
                if not (marker_added)
                  then do
                          ...
                          foldlM
                            ( \res e@(_, floc) -> case floc of 
                                                 ...
                                                 SoA _ flds -> do 
                                                               ...
                                                               let (CursorArrayTy sz) = getCursorizeTyFromLocVar Nothing optTailCall floc
                    ...
                    let let_mk_cur_arr = LetE (after_soa_loc, [], getCursorizeTyFromLocVar Nothing optTailCall (SoA "" aft_flocs), makeCurArr)
              ...
              go2 marker_added fvarenv aft_dloc from_rec_end aft_flocs (((dcon, index), floc, (rnd, (MkTy2 ty))) : rst) = do
                ...
                case ty of
                  PackedTy _ l -> do
                    let cur_ty = getCursorizeTyFromLocVar Nothing optTailCall l
                    (rnd', fvarenv', m1', m2') <- go m1 m2 fvarenv tenv senv rnd
                    end_scalars_alloc <- gensym "end_scalars_alloc"
                    ( if not marker_added
                        then LetE (end_scalars_alloc, [], ProdTy [], Ext $ EndScalarsAllocation (sloc))
                        else id
                      ...
                      <$> case (isMutModality $ fromJust $ modality_sloc) of
                                    False -> LetE (d', [], cur_ty, projEnds rnd') <$> go2 True fvarenv' aft_dloc (Just d') aft_flocs rst
                                    True -> go2 True fvarenv' aft_dloc (Just d') aft_flocs rst
                  _ | isScalarTy ty -> do
                    (rnd', fvarenv', m1', m2') <- cursorizeExp m1 m2 optTailCall fvarenv lenv ddfs fundefs denv tenv senv rnd
                    ...
                    let_assign_write_cur
                      <$> LetE (d', [], CursorTy, Ext $ WriteScalar (mkScalar ty) write_scalars_at rnd')
                  ...
                  VectorTy el_ty -> do
                    ...
                    let fvarenv'' = M.insert (fromLocVarToFreeVarsTy $ singleLocVar $ d') d' fvarenv'
                    let_assign_write_cur
                      <$> LetE (d', [], CursorTy, Ext $ WriteVector write_vector_at rnd' (stripTyLocs el_ty))
                      <$> go2 marker_added fvarenv'' aft_dloc from_rec_end aft_flocs' rst
                  ...
                  CursorTy -> do
                    ...
                    let rnd_ty = case M.lookup rnd_var tenv of 
                                              ...
                                              Just ty -> case unTy2 ty of  
                                                        PackedTy _ l -> do
                                                                         getCursorizeTyFromLocVar'' Nothing optTailCall l
                  ...
                  CursorArrayTy _size -> do
                    ...
                    let rnd_ty = case M.lookup rnd_var tenv of 
                                              ...
                                              Just ty -> case unTy2 ty of  
                                                        PackedTy _ l -> do
                                                                         case l of
                                                                              Single _ -> getCursorizeTyFromLocVar Nothing optTailCall l
                                                                              SoA _ fields -> getCursorizeTyFromLocVar Nothing optTailCall l
                                                        CursorArrayTy sz -> CursorArrayTy sz
                                                        _ -> CursorTy
                    if isIndirectionTag dcon
                    then do 
            ...
            foldlM
              ( \(b, env, idx') ((_, _), loc) -> do
                  ...
                  let b' =
                        if present'
                          then b
                          else b ++ [(var_for_loc, [], getCursorizeTyFromLocVar Nothing optTailCall sloc_loc, Ext $ IndexCursorArray sloc idx')]
                  pure (b', env', idx' + 1)
              )
          ...
          (,,,) <$> dl
            ...
            <$> LetE (start_tag_alloc, [], ProdTy [], Ext $ StartTagAllocation (sloc))
            <$> LetE (writetag, [], (getCursorizeTyFromLocVar Nothing optTailCall (getDconLoc sloc_loc)), Ext $ WriteTag dcon (sloc_dcon))
            <$> LetE (end_tag_alloc, [], ProdTy [], Ext $ EndTagAllocation (sloc))
            <$> LetE (start_scalars_alloc, [], ProdTy [], Ext $ StartScalarsAllocation (sloc))
            <$> LetE (after_tag, [], getCursorizeTyFromLocVar Nothing optTailCall (getDconLoc sloc_loc), Ext $ AddCursor (sloc_dcon) (L3.LitE 1))
    ...
    TimeIt e t b -> do
      (Di e', freeVarToVarEnv', m1', m2') <- go m1 m2 freeVarToVarEnv tenv senv e
      return (Di $ TimeIt e' (cursorizeTy m1' m2' optTailCall Nothing (unTy2 t)) b, freeVarToVarEnv', m1', m2')
    WithArenaE v e -> do
      (Di e', freeVarToVarEnv', m1', m2') <- go m1 m2 freeVarToVarEnv (M.insert v (MkTy2 ArenaTy) tenv) senv e
    ...
    Ext ext ->
      case ext of
            ...
            Right (rhs', bnds', bnds_after, tenv'', senv') -> do
              ...
              let locs_var = case (M.lookup (fromLocVarToFreeVarsTy loc) freeVarToVarEnv') of
                    ...
                    Nothing -> case loc of
                      Single lvarrr -> lvarrr
                      SoA _ _ -> error "cursorizeExp: LetLocE: unexpected location variable"
              let locs_ty3 :: Ty3 = getCursorizeTyFromLocVar modality optTailCall loc
              let locs_ty2 = getCursorizeTyFromLocVar' modality optTailCall loc
              case rhs of
                FromEndLE {} ->
                  if isBound locs_var tenv
              ...
              SoARv _ _ -> case ((isMutModality (fromJust modality))) of 
                ...
                False -> if ((M.member (fromRegVarToFreeVarsTy loc) freeVarToVarEnv))
                         then do
                          ...
                          name <- gensym "cursor_ptr"
                          return $ M.insert (fromRegVarToFreeVarsTy loc) name freeVarToVarEnv
          (rhs_either, m1', m2') <- cursorizeRegExp m1 m2 optTailCall freeVarToVarEnv' denv tenv senv loc rhs
          let (bnds, tenv') = case M.lookup (fromRegVarToFreeVarsTy loc) denv of
                Nothing -> ([], tenv)
                Just vs ->
                  let extended = M.fromList [(v, MkTy2 CursorTy) | (v, _, CursorTy, _) <- vs]
            ...
            Right (rhs', bnds', tenv'', senv') -> do
              ...
              case rhs of
                ...
                _ -> do
                  case ty_of_loc of
                    MutCursorTy -> do
                      (bod', freeVarToVarEnv'', m1'', m2'') <- go m1' m2' freeVarToVarEnv' (M.insert locs_var (ty2_of_loc) tenv''') senv' bod 
        ...
        TagCursor a b -> do
          ...
          tag_cur_var <- gensym "tag_cur"
          casted_var <- gensym "cast"
          let ty3_of_field = getCursorizeTyFromLocVar Nothing optTailCall (toLocVar a)
          let ty3_of_field2 :: Ty3 = getCursorizeTyFromLocVar'' Nothing optTailCall (toLocVar a)
        ...
        RetE locs v -> do
          if optTailCall 
          then do
            (v', freeVarToVarEnv', m1', m2') <- go m1 m2 freeVarToVarEnv tenv senv (VarE v)
            case locs of
          ...
          if gopt Opt_DisableGC dflags
            ...
            then do
              let locs_var = case M.lookup (fromLocArgToFreeVarsTy to) freeVarToVarEnv of
                    ...
                    Just var -> var
              go m1 m2 freeVarToVarEnv tenv senv (DataConE from dcon [VarE locs_var])
            else do
...
mutLocNeedsBump freeVarToVarEnv ptsEnv oldEnv lv v offset = do 
                                       let mutBump = L.foldr (\(kl, (vpts, lpts, _endreg, _aliases)) mred -> case (lv, v) of 
...
cursorizeReadPackedFile ::
  ...
  Exp2 ->
  PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeReadPackedFile m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv isPackedContext v path tyc reg ty2 bod = do
  case reg of
    Nothing -> error $ "cursorizePackedExp: InferLocations did not set the reg for ReadPackedFile."
    Just reg_var -> do 
      (bod', freeVarToVarEnv', m1', m2') <- go m1 m2 (M.insert v (MkTy2 CursorTy) tenv) bod
      return (
        mkLets
  ...
  where
    go m1g m2g t e =
      if isPackedContext
        then 
          do 
           (e', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g optTailCall freeVarToVarEnv lenv ddfs fundefs denv t senv e
           return (fromDi e', freeVarToVarEnv', m1g', m2g') 
        else cursorizeExp m1g m2g optTailCall freeVarToVarEnv lenv ddfs fundefs denv t senv e
...
cursorizeLocExp mLocPtsToEnv mLocOldValEnv optTailCall freeVarToVarEnv denv tenv senv lvararg locExp =
  case locExp of
    AfterConstantLE i loc -> do
      ...
      (locs_var, use_this_loc, additional_bnds, bnds_after, mLocPtsToEnv', mLocOldValEnv') <- case ((isMutModality $ fromJust $ getModality loc) || (Mb.isJust mut_loc_var && (isInputModality (getModality loc)))) of 
                                                                              ...
                                                                              case needs_bump of
                                                                                Nothing -> if (M.member (toLocVar loc) mLocPtsToEnv)
                                                                                            ...
                                                                                            -- value of the OutputMutable location.
                                                                                              ...
                                                                                              let bumpMutLoc = (bump_loc_var, [], ProdTy [], Ext $ BumpCursorMutable loc_name (LitE i))
                                                                                              -- We need to make the mutable loc point to the dereferenced value 
                                                                                              let mLocPtsToEnv'' = M.insert (toLocVar loc) (lvar_name, Just lvar, Just $ toEndRegVar lvararg, S.empty) mLocPtsToEnv 
                                                                                              -- if there is no mapping of the mutable loc to its old value, we need to update it.
                                                                                              let mLocOldValEnv'' = if (M.member (toLocVar loc) mLocOldValEnv)
                                                                                                                  then mLocOldValEnv
                                                                    ...
                                                                    _ -> do
                                                                          let lvar_name = getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy lvar)
                                                                          let loc_name = getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy (toLocVar loc))
                                                                          (new_key_vals, after_bnds) <- foldlM (\(kvals, bnds) (key, (vval, lval, endreg, aliases)) -> case lval of 
                                                                                                                                                  Nothing -> if vval == loc_name
                                                                                                                                                             then
                                                                                                                                                              ...
                                                                                                                                                              return (kvals ++ [(key, (lvar_name, Just lvar, Just $ toEndRegVar lvararg, aliases))], bnds ++ [aft_bnd])
                                                                                                                                                             else return $ (kvals ++ [(key, (vval, lval, endreg, aliases))], bnds)
    ...
    FromEndLE locarg ->
      let loc = toLocVar locarg
       ...
       in if isBound locs_var tenv
            ...
            else pure $ (Left $ M.insertWith (++) (fromLocVarToFreeVarsTy loc) [(lvar_name, [], CursorTy, VarE locs_var)] denv,  mLocPtsToEnv, mLocOldValEnv)
    StartOfRegionLE r -> case r of
      GlobR v _ -> do
                    ...
                    case (getModality lvararg) of 
                            Just OutputMutable -> do
                                            let mLocPtsToEnv' = M.insert lvar (v, Nothing, Just $ toEndVRegVar (regionToVar r), S.empty) mLocPtsToEnv
      ...
      SoAR dr fregs -> do
        let lvar = toLocVar lvararg
        let lvarty = getCursorizeTyFromLocVar'' Nothing optTailCall lvar
        let lvarname = getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy lvar)
        ...
        case (getModality lvararg) of
                Just OutputMutable -> do
                                let mLocPtsToEnv' = M.insert lvar (regions_var, Nothing, Just $ toEndVRegVar (regionToVar r), S.empty) mLocPtsToEnv 
    ...
    GetDataConLocSoA loc -> do
      ...
      (mLocPtsToEnv', mLocOldValEnv', rhs, additional_bnds) <- case isMutModality' modality of 
                                                True -> do 
                                                                      ...
                                                                      then do 
                                                                          cpy <- gensym "cpy"
                                                                          let cpy_ty = getCursorizeTyFromLocVar'' modality optTailCall loc_from_logarg
                                                                          ...
                                                                          let mLocPtsToEnv_i = M.insert loc_from_logarg (cpy, Nothing, Just loc_region, S.empty) mLocPtsToEnv
                    ...
                    (\(_, loc) -> let modality_floc = getModality loc
                                   ...
                                   in case (toLocVar loc) of 
                                        ...
                                        SoA{} -> let var_for_loc = case (M.lookup (fromLocVarToFreeVarsTy (toLocVar loc)) freeVarToVarEnv) of
                                                                    Just v -> v
                                                                    Nothing -> error "cursorizeLocExp: GenSoALoc: unexpected field location variable"
                                                     loc_ty = getCursorizeTyFromLocVar Nothing optTailCall (toLocVar loc)
                                                  in case loc_ty of 
       ...
       then dbgTrace (minChatLvl) "Print in findMutSoALocPtsToSoALoc: " dbgTrace (minChatLvl) (sdoc (dconloc, dconlocmut, fldlocs, fldlocs')) dbgTrace (minChatLvl) "End in findMutSoALocPtsToSoALoc!\n" env -- error $ "Did not expected null!" ++ show (dconloc, dconlocmut, fldlocs, fldlocs')
        ...
        let constructed_soa_loc = SoA (unwrapLocVar $ fromJust dconlocmut) (map (\(k, l) -> (k, fromJust l)) fldlocs')
         ...
         in case mutsoa of 
                    ...
                    Just (_var, _l, r, aliases) -> dbgTrace (minChatLvl) "Print in findMutSoALocPtsToSoALoc: " dbgTrace (minChatLvl) (sdoc (dconloc, dconlocmut, fldlocs, fldlocs')) dbgTrace (minChatLvl) "End Not null in findMutSoALocPtsToSoALoc!\n" M.insert constructed_soa_loc (locsvar, Just locvar, r, S.insert _var aliases) env
...
cursorizeRegExp mLocPtsToEnv mLocOldValEnv optTailCall freeVarToVarEnv denv tenv senv lvar regExp =
  case regExp of
            ...
            mapM
              (\(_, loc) -> case (fromLocVarToRegVar $ toLocVar loc) of 
                                  ...
                                  SoARv{} -> let var_for_loc = case (M.lookup (fromRegVarToFreeVarsTy (fromLocVarToRegVar $ toLocVar loc)) freeVarToVarEnv) of
                                                              Just v -> v
                                                              Nothing -> error "cursorizeLocExp: GenSoALoc: unexpected field location variable"
                                                 loc_ty = getCursorizeTyFromRegVar Nothing optTailCall (fromLocVarToRegVar $ toLocVar loc)
                                             in case loc_ty of 
                                                    CursorTy -> pure $ [(var_for_loc, [])]
                                                    CursorArrayTy sz -> do
                                                                         indexing_inst <- foldlM (\new_names i -> do
...
cursorizeAppE :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeAppE m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv ex =
  case ex of
                    ...
                    SoARv dconReg fieldRegions -> do
                      ...
                      ret <- case parentRegion of
                        Just par_reg -> do
                          ...
                          let (start, end, _) = getIndexPositionOfSoARegVar optTailCall Nothing (getAllFieldRegsSoA par_reg_inner) r
                          (_acc, instrs) <- handleIndexingSoARegCursors optTailCall (r, name) (start, end) par_reg_inner acc
                          ...
                          (fieldReg_vars, bnds) <-
                                foldlM
                                  (\(vs, bds) (key, field_reg) -> do 
                                                                    ...
                                                                    case field_reg of 
                                                                          SingleR{} -> do 
                                                                                       let (idx, _, _) = getIndexPositionOfSoARegVar optTailCall Nothing fieldRegions field_reg
                                                                                       pure (vs ++ [v], bds ++ [(v, [], CursorTy, Ext $ IndexCursorArray v idx)])
                                                                          SoARv{} -> do
                                                                                     --let (start, end, _) = getIndexPositionOfSoARegVar fieldRegions field_reg
                                                                                     let CursorArrayTy _sz = getCursorizeTyFromRegVar Nothing optTailCall field_reg
                                  ...
                                  fieldRegions
                          name <- gensym "cursor_reg_ptr"
                          let instrs = dcon_insts ++ (map (\i -> LetE i) bnds) ++ [LetE (name, [], getCursorizeTyFromRegVar Nothing optTailCall r, Ext $ MakeCursorArray (1 + length fieldReg_vars) ([dconReg_var] ++ fieldReg_vars))]
        ...
        mapM
          ( \(t, a) ->
              if hasPacked (unTy2 t)
                then 
                  do 
                    (a', _, _, _) <- cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv' lenv ddfs fundefs denv tenv senv a
                    fromDi <$> return a'
                else do 
                    (a', _, _, _) <- cursorizeExp m1 m2 optTailCall freeVarToVarEnv' lenv ddfs fundefs denv tenv senv a
                    return a'
          )
          (zip in_tys args)
      let starts = zipWith (giveStarts freeVarToVarEnv optTailCall m1 m2) (map unTy2 argTys) args'
      -- let loc_var = toLocVar loc
                        ...
                        case mutl of 
                                  ...
                                  Nothing -> do 
                                             case (getModality loc) of 
                                                      Nothing -> do 
                                                                 return $ (bnds, args ++ [VarE (getVarNameFromFreeVar freeVarToVarEnv' (fromLocArgToFreeVarsTy loc))])
                                                      Just Input -> do 
                                                      ...
                                                      Just _ -> return $ (bnds, args ++ [VarE (getVarNameFromFreeVar freeVarToVarEnv' (fromLocArgToFreeVarsTy loc))]) -- [VarE (getVarNameFromFreeVar freeVarToVarEnv' (fromLocArgToFreeVarsTy loc))]
                                  Just mutl' -> case (isRegionLocArg loc) of 
                                                          False -> return $ (bnds, args ++ [VarE (getVarNameFromFreeVar freeVarToVarEnv' (fromLocVarToFreeVarsTy mutl'))])
                                                          True -> dbgTrace (minChatLvl) "Print in true case outs: " dbgTrace (minChatLvl) (sdoc (mutl')) dbgTrace (minChatLvl) "End in true case out." return $ (bnds, args ++ [VarE (getVarNameFromFreeVar freeVarToVarEnv' (fromRegVarToFreeVarsTy $ L2.fromLocVarToRegVar mutl'))])
                    ) ([], []) outs
...
-}
cursorizeProj :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeProj m1 m2 optTailCall freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex =
  case ex of
    LetE (v, _locs, ty, rhs@ProjE {}) bod | isPackedTy (unTy2 ty) -> do
      (rhs', freeVarToVarEnv', m1', m2') <- go m1 m2 tenv rhs
      let ty' = gRecoverType ddfs (Env2 tenv M.empty) rhs
          ty'' = cursorizeTy m1 m2 optTailCall Nothing (unTy2 ty')
          bnds =
            if isPackedTy (unTy2 ty')
              then
      ...
      (bod', freeVarToVarEnv'', m1', m2') <- go m1 m2 tenv' bod
  ...
  where
    go m1g m2g t x =
      if isPackedContext
        then do 
           (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g optTailCall freeVarToVarEnv lenv ddfs fundefs denv t senv x
           return (fromDi x', freeVarToVarEnv', m1g', m2g')
        else cursorizeExp m1g m2g optTailCall freeVarToVarEnv lenv ddfs fundefs denv t senv x
...
-}
cursorizeProd :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeProd m1 m2 optTailCall freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex =
  case ex of
    LetE (v, _locs, MkTy2 (ProdTy tys), rhs@(MkProdE ls)) bod -> do
      ...
      (bod', env1, m1', m2') <- go m1 m2 tenv' bod
  ...
  where
    go m1 m2 t x =
      if isPackedContext
        then do
           (x', freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv t senv x
           return (fromDi x', freeVarToVarEnv', m1', m2') 
        else cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv t senv x
...
-}
cursorizeSpawn :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeSpawn m1 m2 optTailCall freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex = do
  case ex of
    LetE (v, locs, MkTy2 ty, (SpawnE fn applocs args)) bod
      | isPackedTy ty -> do
          (rhs', freeVarToVarEnv', m11, m22) <- do 
                  (expr, freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv (AppE fn UnknownTailType applocs args)
          ...
          let ty' = case locs of
                [] -> cursorizeTy m11 m22 optTailCall Nothing ty
                xs -> ProdTy ([CursorTy | _ <- xs] ++ [cursorizeTy m11 m22 optTailCall Nothing ty])
          ...
          (bod', freeVarToVarEnv'', m11', m22') <- go m11 m22 tenv' senv' bod
          let bod'' = updateAvailVars [v] [fresh] bod'
          return (mkLets bnds bod'', M.union freeVarToVarEnv' freeVarToVarEnv'', m11', m22')
      | hasPacked ty -> do
          (rhs', freeVarToVarEnv', m11, m22) <- do 
                   (expr, freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv (AppE fn _cty applocs args)
                   return (fromDi expr, freeVarToVarEnv', m1', m2') 
          let rhs'' = case rhs' of
          ...
          let ty' = case locs of
                [] -> cursorizeTy m11 m22 optTailCall Nothing ty
                xs -> ProdTy ([CursorTy | _ <- xs] ++ [cursorizeTy m1 m2 optTailCall Nothing ty])
              ty'' = stripTyLocs ty'
          ...
          case locs of
            ...
            _ -> do
              ...
              return (mkLets bnds bod', M.union freeVarToVarEnv' freeVarToVarEnv'', m11', m22') 
      | otherwise -> do
          (rhs', freeVarToVarEnv', m1', m2') <- cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv (AppE fn UnknownTailType applocs args)
          let rhs'' = case rhs' of
                AppE fn' _ applocs' args' -> SpawnE fn' applocs' args'
                _ -> error "cursorizeSpawn"
          case locs of
            [] -> do 
                   (bod', freeVarToVarEnv'', m1'', m2'') <- go m1' m2' (M.insert v (MkTy2 ty) tenv) senv bod
                   return (LetE (v, [], curDict $ stripTyLocs ty, rhs'') bod', M.union freeVarToVarEnv'' freeVarToVarEnv', m1'', m2'')
            [loc] -> do
              fresh <- gensym "par_tup_scalar"
              let ty' :: OldTy2
                  ty' = ProdTy ([CursorTy | _ <- locs] ++ [cursorizeTy m1' m2' optTailCall Nothing ty])
                  tenv' = M.union (M.fromList [(fresh, MkTy2 ty')]) tenv
                  ty'' :: Ty3
                  ty'' = stripTyLocs ty'
                  rhs''' = Di (VarE fresh)
                  ...
                  senv' = M.insert v pending_bnds senv
              (bod', freeVarToVarEnv'', m1'', m2'') <- go m1' m2' tenv' senv' bod
  ...
  where
    go m1g m2g t s x =
      if isPackedContext
        then do 
            (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g optTailCall freeVarToVarEnv lenv ddfs fundefs denv t s x
            return (fromDi x', freeVarToVarEnv', m1g', m2g')
        else cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv t s x

cursorizeSync :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> Bool -> M.Map FreeVarsTy Var -> M.Map Var (Maybe LocVar) -> Bool -> DDefs Ty2 -> FunDefs2 -> DepEnv -> TyEnv Var Ty2 -> SyncEnv -> Exp2 -> PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeSync m1 m2 optTailCall freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv ex = do
  case ex of
  ...
  where
    go m1g m2g t x =
      if isPackedContext
        then do 
           (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g optTailCall freeVarToVarEnv lenv ddfs fundefs denv t M.empty x
           return (fromDi x', freeVarToVarEnv', m1g', m2g') 
        else cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv t M.empty x
...
cursorizeLet ::
  ...
  Exp2 ->
  PassM (Exp3, M.Map FreeVarsTy Var, MutableLocPtsToEnv, MutableLocOldValueEnv)
cursorizeLet m1 m2 optTailCall freeVarToVarEnv lenv isPackedContext ddfs fundefs denv tenv senv (v, locs, (MkTy2 ty), rhs) bod
  | isPackedTy ty = do
              ...
              ( \loc ->
                  let free_var = fromLocArgToFreeVarsTy loc
                      cursorType = case free_var of
                        R r -> getCursorizeTyFromRegVar'' Nothing optTailCall r
                        V _ -> error "cursorizeLet: did not expect a variable in locations in a LetE."
                        FL l -> getCursorizeTyFromLocVar'' Nothing optTailCall l
                   in cursorType
              )
              locs
      let cursor_ty_locs' =
      ...
      -- Vidush: TODO:         
      let ty' = case locs of
            [] -> cursorizeTy m1' m2' optTailCall Nothing ty
            xs -> if optTailCall
                  then cursorizeTy m1' m2' optTailCall Nothing ty
                  else ProdTy (cursor_ty_locs ++ [cursorizeTy m1' m2' optTailCall Nothing ty])
  
          -- Probably do not need the locs to be added in the environment.         
          tys_in_env = if optTailCall 
      ...
      bnds <- case locs of
            [] -> if M.member start_loc m2
                  ...
                  then
                    ...
                    let type_l2 = getCursorizeTyFromLocVar Nothing optTailCall start_loc
                    new_deref <- gensym "deref"
                    case start_loc of 
                          Single{} -> pure [ (fresh, [], ty'', rhs'),
                              ...
                              Just (l, endreg) -> do
                                        ...
                                        case start_loc of 
                                              Single{} -> pure [ (fresh, [], ty'', rhs'),
                                                              ...
                                                              (v, [], getCursorizeTyFromLocVar Nothing optTailCall start_loc, VarE start_var), 
                                                              ...
                                                              (toEndV v, [], getCursorizeTyFromLocVar Nothing optTailCall start_loc, VarE new_deref) 
                                                              --()
                                                            ] 
                                              SoA{} -> pure [ (fresh, [], ty'', rhs'),
                                                              --(v, [], projTy 0 ty'', mkProj 0 rhs''),
            ...
            _ -> if M.member start_loc m2'
                  ...
                  then
                    ...
                    new_deref <- gensym "deref"
                    let loc_bnds = concatMap (\(loc, n) -> let loc_var = fromLocArgToFreeVarsTy loc
                                                                                    ...
                                                                                    in case loc of 
                                                                                        ...
                                                                                        EndOfReg r m er -> if m == Output
                                                                                                           then case regVarLoc of
                                                                                                                    SingleR{} -> [(loc_to_variable, [], CursorTy, Ext $ DerefMutCursor varNameReg)] 
                                                                                                                    SoARv{} ->  [(loc_to_variable, [], cursor_ty, VarE varNameReg)]
                                                                                                           else []
                 ...
                 then 
                  -- find the output mutable location that points to the start_var of the packed ty
                  let mut_loc = L.foldr (\(ml, (vv, vl, erg, _aliases)) res -> if vv == start_var
                                                                  then Just (ml, erg)
                                                                  else res
                                          ) Nothing (M.toList m1)
                    in case mut_loc of
                  ...
                  --                 ) (zip locs [0 ..])
                            Just (l, endreg) -> do 
                                      new_deref <- gensym "deref"
                                      let tylocvar = getCursorizeTyFromLocVar Nothing optTailCall start_loc
                                      let varName = (getVarNameFromFreeVar freeVarToVarEnv (fromLocVarToFreeVarsTy l))
                                      let loc_bnds = concatMap (\(loc, n) -> let loc_var = fromLocArgToFreeVarsTy loc
                                                                                 cursor_ty = cursor_ty_locs' !! n
                                                                                 loc_to_variable = case (M.lookup (loc_var) freeVarToVarEnv') of
                  ...
                  let nLocs = length locs
                    ...
                    in pure (bnds' ++ locBnds) 
      case M.lookup (fromVarToFreeVarsTy (toEndV v)) denv of
        Just xs -> error $ "todo: " ++ sdoc xs
        Nothing -> return ()
      (bod', freeVarToVarEnv'', m1'', m2'') <- go m1' m2' (M.union freeVarToVarEnv' freeVarToVarEnv) tenv' bod
      return (mkLets bnds bod', freeVarToVarEnv'', m1'', m2'')
  | hasPacked ty = do
      let cursor_ty_locs =
            map
      ...
      let cursor_ty_locs' =
            map
              ...
              locs
      (_rhs, freeVarToVarEnv', m1', m2') <- cursorizePackedExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv rhs
      rhs' <- fromDi <$> return _rhs 
      fresh <- gensym "tup_haspacked"
      let ty' = case locs of
            [] -> cursorizeTy m1' m2' optTailCall Nothing ty
            xs -> ProdTy (cursor_ty_locs ++ [cursorizeTy m1' m2' optTailCall Nothing ty])
          ty'' = stripTyLocs ty'
          tenv' =
            M.union
  ...
  | otherwise = do
      ...
      let cursor_ty_locs' =
            map
              ...
              locs
      (rhs', freeVarToVarEnv', m1', m2') <- cursorizeExp m1 m2 optTailCall freeVarToVarEnv lenv ddfs fundefs denv tenv senv rhs
      case locs of
        [] -> do 
          (bod', freeVarToVarEnv'', m1'', m2'') <- go m1' m2' (M.union freeVarToVarEnv' freeVarToVarEnv) (M.insert v (MkTy2 ty) tenv) bod
          return (LetE (v, [], curDict $ stripTyLocs ty, rhs') bod', freeVarToVarEnv'', m1'', m2'')
        _ -> do
          fresh <- gensym "tup_scalar"
          let rhs'' = VarE fresh
              ty' = ProdTy (cursor_ty_locs ++ [cursorizeTy m1 m2 optTailCall Nothing ty])
  ...
  where
    go m1g m2g fenv t x =
      if isPackedContext
        then
          do 
            (x', freeVarToVarEnv', m1g', m2g') <- cursorizePackedExp m1g m2g optTailCall fenv lenv ddfs fundefs denv t senv x
            return (fromDi x', freeVarToVarEnv', m1g', m2g') 
        else cursorizeExp m1g m2g optTailCall fenv lenv ddfs fundefs denv t senv x
...
unpackDataCon m1 m2 optTailCall dcon_var freeVarToVarEnv lenv ddfs fundefs denv1 tenv1 senv isPacked scrtCur (dcon, vlocs1, rhs) = do
  ...
  case ty_of_scrut of
    ...
    CursorArrayTy size -> do
      ...
      (field_lets, field_v_lst, freeVarToVarEnv', m1', m2') <-
        dbgTrace
          (minChatLvl)
          "Print scrut_loc "
          dbgTrace
          ...
          "end scrut_loc.\n"
          foldlM
          ( \(acc1, acc2, acc3, mi1, mi2) (key@(dcon', idx), loc) -> do
              ...
              let field_cursor_ty = getCursorizeTyFromLocVar Nothing optTailCall loc
              let (start, end, _) = getIndexPositionOfSoALocVar optTailCall Nothing (getAllFieldLocsSoA scrut_loc) loc
              (field_var', acc3'', field_let, mi1', mi2') <- handleIndexingSoACursors case_var_loc mi1 mi2 optTailCall (loc, field_var) (start, end) scrut_loc acc3'
              ...
              return (acc1 ++ field_let, acc2 ++ curr_window, acc3'', mi1', mi2')
          )
          ([], [], freeVarToVarEnv, m1, m2)
      ...
      bod <-
        ( if isAbsRANDataCon dcon
            then do      
              ...
              if isRelRANDataCon dcon
                then unpackWithRelRAN field_cur
                else unpackRegularDataCon m1' m2' (SoAWin dcon_var field_v_lst) freeVarToVarEnv'
        )
        ...
        (field_lets, field_v_lst, freeVarToVarEnv', m1', m2') <-
          dbgTrace
            ...
            ( \(acc1, acc2, acc3, mi1, mi2) (key@(dcon', idx), loc) -> do
                ...
                (field_var', field_let) <- case scrut_loc_is_mutable of
                                                  True -> do 
                                                           deref_field_var <- gensym "deref"
                                                           let field_cursor_ty' = getCursorizeTyFromLocVar (Just OutputMutable) optTailCall loc
                                                           return (deref_field_var, [(deref_field_var, [], field_cursor_ty', Ext $ AddrOfCursor $ Ext $ IndexCursorArray scrtCur (1 + idx_elem))])
                                                  False -> return (field_var, [(field_var, [], field_cursor_ty, Ext $ IndexCursorArray scrtCur (1 + idx_elem))])
  ...
  where
    ...
    --
    unpackRegularDataCon :: MutableLocPtsToEnv -> MutableLocOldValueEnv -> WindowIntoCursor -> M.Map FreeVarsTy Var -> PassM Exp3
    unpackRegularDataCon m1 m2 field_cur freeVarToVarEnv_unpack = do
                  ...
                  let loc = fromLocArgToFreeVarsTy locarg
                   ...
                   in case ty of
                        ...
                        _ | isScalarTy ty -> do
                          ...
                          let cur = dbgTrace (minChatLvl) "Print in scalar ty: " dbgTrace (minChatLvl) (sdoc (binds, loc_var, loc)) dbgTrace (minChatLvl) "End in scalar ty SoA unpackDcon!\n." fromJust $ L.lookup (dcon, field_idx) _field_cur
                          if canBind
                            then do
                              -- If the location exists in the environment, it indicates that the
                              -- corresponding variable was also bound and we shouldn't create duplicate
                              ...
                              res <- mapM (\((dcon, idx), _loc) -> do
                                                                               ...
                                                                               case locTy of 
                                                                                                MkTy2 (PackedTy _ loc) -> do
                                                                                                                          let lty = getCursorizeTyFromLocVar Nothing optTailCall loc
                                                                                                                          case lty of 
                                                                                                                               CursorTy -> do
                        ...
                        PackedTy tycon ploc -> do
                          ...
                          case isSameTycon of
                            True -> do
                              let ty3_of_field = getCursorizeTyFromLocVar' Nothing optTailCall ploc
                              let ty3_of_field2 :: Ty3 = getCursorizeTyFromLocVar Nothing optTailCall ploc
                ...
                ((v, locarg) : rst_vlocs, (MkTy2 ty) : rst_tys) ->
                  let loc = fromLocArgToFreeVarsTy locarg
                   in case ty of
                        ...
                        _ | isScalarTy ty -> do
                          locs_var <- lookupVariable loc fenv
                          (tenv', binds) <- scalarBinds ty v locs_var locarg tenv
          ...
          case (vlocs, tys) of
            ...
            ((v, locarg) : rst_vlocs, (MkTy2 ty) : rst_tys) ->
              let loc = toLocVar locarg
                  ...
                  locsTy2 = getCursorizeTyFromLocVar' Nothing optTailCall loc
                  locs_var = case (M.lookup (fromLocVarToFreeVarsTy loc) freeVarToVarEnv) of
                    Just v' -> v'
                    Nothing -> error "cursorizeLet: unexpected location variable"
               in case ty of
    ...
    scalarBinds :: OldTy2 -> Var -> Var -> LocArg -> TyEnv Var Ty2 -> PassM (TyEnv Var Ty2, [(Var, [()], Ty3, Exp3)])
    scalarBinds ty v loc lcarg tenv = do
      tmp <- gensym "read_scalar_tuple"
      let locsTy2 = getCursorizeTyFromLocVar (getModality lcarg) optTailCall (toLocVar lcarg)
          locsTy3 = getCursorizeTyFromLocVar' (getModality lcarg) optTailCall (toLocVar lcarg)
          locsUrTy = getCursorizeTyFromLocVar'' (getModality lcarg) optTailCall (toLocVar lcarg)
...
giveStarts :: M.Map FreeVarsTy Var -> Bool -> MutableLocPtsToEnv -> MutableLocOldValueEnv -> OldTy2 -> Exp3 -> Exp3
giveStarts fenv optTailCall mlocptsenv moldenv ty e =
  case ty of
    PackedTy _ loc -> if optTailCall
                      then case M.lookup loc moldenv of 
                                        Nothing -> error $ "Expected to have loc in env!!" ++ show (loc, moldenv)
                                        Just (oldv, oldl, _, _) -> dbgTrace (minChatLvl) "Print in give starts: " dbgTrace (minChatLvl) (sdoc (loc, oldv)) dbgTrace (minChatLvl) "End in give starts!\n" VarE oldv
                      else mkProj 0 e
    -- NOTE : mkProj . MkProdE == id
    ProdTy tys -> MkProdE $ zipWith (\ty' n -> giveStarts fenv optTailCall mlocptsenv moldenv ty' (mkProj n e)) tys [0 ..]
    CursorArrayTy{} -> case e of 
                          VarE v -> let mutloc = findMutableLocationPointingToVar v mlocptsenv
                                     in case mutloc of 
                                              ...
                                              Just ml -> let mlVarName = dbgTrace (minChatLvl) "Print in give starts: " dbgTrace (minChatLvl) (sdoc (v, ml, mlocptsenv)) dbgTrace (minChatLvl) "End in give starts Just ml!\n" getVarNameFromFreeVar fenv (fromLocVarToFreeVarsTy ml)
                                                          in VarE mlVarName
                          _ -> e  
    _ -> e
