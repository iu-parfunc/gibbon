...
instance (FreeVars (e l d), Out l, Out d) => FreeVars (PreExp e l d) where
  gFreeVars ex =
    case ex of
      ...
      LetE (v, _, _, rhs) bod ->
        gFreeVars rhs `S.union` S.delete v (gFreeVars bod)
      CaseE e ls ->
        S.union
          (gFreeVars e)
          (S.unions $
...
instance Renamable Var
 where
  gRename env v = M.findWithDefault v v env

instance HasSubstitutable e l d => Substitutable (PreExp e l d)
 where
  gSubst  = subst
  gSubstE = substE
  ...
  gRename env ex =
    case ex of
      ...
      LetE (v, locs, ty, rhs) bod ->
        LetE (go v, gol locs, go ty, go rhs) (go bod)
      ...
      CaseE scrt ls ->
        CaseE
          (go scrt)
          (map (\(a, b, c) -> (a, map (\(d, e) -> (go d, go e)) b, go c)) ls)
...
instance Renamable a => Renamable (UrTy a)
 where
...
mapExprs fn prg@Prog {fundefs, mainExp} =
  let mainExp' =
   ...
   in prg
        { fundefs = M.map (\g -> g {funBody = fn (funBody g)}) fundefs
        , mainExp = mainExp'
        }
...
subst old new ex =
  let go = subst old new
   in case ex of
        ...
        LetE (v, loc, t, rhs) bod
          | v == old -> LetE (v, loc, t, go rhs) bod
          | otherwise -> LetE (v, loc, t, go rhs) (go bod)
        ProjE i e -> ProjE i (go e)
        CaseE e ls -> CaseE (go e) (L.map f ls)
          where f (c, vs, er) =
                  if L.elem old (L.map fst vs)
                    then (c, vs, er)
                    else (c, vs, go er)
...
substE old new ex =
  let go = substE old new
   in case ex of
        ...
        PrimAppE p ls -> PrimAppE p $ L.map go ls
        LetE (v, loc, t, rhs) bod
          | (VarE v) == old -> LetE (v, loc, t, go rhs) bod
          | otherwise -> LetE (v, loc, t, go rhs) (go bod)
        ProjE i e -> ProjE i (go e)
        ...
        SyncE -> SyncE
        MapE (v, t, rhs) bod
          | VarE v == old -> MapE (v, t, rhs) (go bod)
          | otherwise -> MapE (v, t, go rhs) (go bod)
        FoldE (v1, t1, r1) (v2, t2, r2) bod ->
          let r1' =
                if VarE v1 == old
...
hasTimeIt rhs =
  case rhs of
    ...
    FoldE (_, _, e1) (_, _, e2) e3 ->
      hasTimeIt e1 || hasTimeIt e2 || hasTimeIt e3
    Ext _ -> False
...
projNonFirst ::
     (Out l, Out d, Out (e l d)) => Int -> (PreExp e l d) -> (PreExp e l d)
projNonFirst 0 e =
  error $ "projNonFirst: expected nonzero index into expr: " ++ sdoc e
...
mkLetE (vr, lvs, ty, (LetE bnd e)) bod =
  mkLetE bnd $ mkLetE (vr, lvs, ty, e) bod
...
tuplizeRefs ref vars tys =
  mkLets $
  L.map
    (\(v, ty, ix) -> (v, [], ty, mkProj ix (VarE ref)))
    (L.zip3 vars tys [0 ..])
