...
instance NFData a => NFData (DDef a)
...
getConOrdering dd tycon = L.map fst dataCons
  where
    DDef {dataCons} = lookupDDef dd tycon
...
lookupDataCon' ddf@DDef {dataCons} con =
  case L.filter ((== con) . fst) dataCons of
    [] ->
      error $
      "lookupDataCon': could not find constructor " ++
      show con ++ ", in datatype:\n  " ++ sdoc ddf
    [hit] -> L.map snd (snd hit)
    _ ->
      error $
      "lookupDataCon': found multiple occurences of constructor " ++
      show con ++ ", in datatype:\n  " ++ sdoc ddf
...
insertDD d = M.insertWith err' (tyName d) d
  where
    err' =
      error $
      "insertDD: data definition with duplicate name: " ++ show (tyName d)
...
class (Out (ArrowTy ty), Show (ArrowTy ty)) =>
      FunctionTy ty
  where
  type ArrowTy ty
  inTys :: ArrowTy ty -> [ty]
...
deriving instance
         (Read ex, Read (ArrowTy (TyOf ex))) => Read (FunDef ex)

deriving instance
         (Show ex, Show (ArrowTy (TyOf ex))) => Show (FunDef ex)

deriving instance (Eq ex, Eq (ArrowTy (TyOf ex))) => Eq (FunDef ex)

deriving instance
         (Ord ex, Ord (ArrowTy (TyOf ex))) => Ord (FunDef ex)

deriving instance Generic (FunDef ex)

deriving instance
         (Generic (ArrowTy (TyOf ex)), NFData ex,
          NFData (ArrowTy (TyOf ex))) =>
         NFData (FunDef ex)

deriving instance
         (Generic (ArrowTy (TyOf ex)), Out ex, Out (ArrowTy (TyOf ex))) =>
         Out (FunDef ex)
...
insertFD d = M.insertWith err' (funName d) d
  where
    err' =
      error $
      "insertFD: function definition with duplicate name: " ++ show (funName d)
...
--
deriving instance
         (Read (TyOf ex), Read ex, Read (ArrowTy (TyOf ex))) =>
         Read (Prog ex)
...
deriving instance
         (Eq (TyOf ex), Eq ex, Eq (ArrowTy (TyOf ex))) => Eq (Prog ex)

deriving instance
         (Ord (TyOf ex), Ord ex, Ord (ArrowTy (TyOf ex))) => Ord (Prog ex)
...
getFunTy fn Prog {fundefs} =
  case M.lookup fn fundefs of
    ...
    Nothing ->
      error $
      "getFunTy: L1 program does not contain binding for function: " ++ show fn

instance ( Generic (ArrowTy (TyOf ex))
         , Out (ArrowTy (TyOf ex))
         , Out (TyOf ex)
         , Out ex
         ) =>
...
deriving instance
         (Show (TyOf a), Show a, Show (ArrowTy a)) => Show (Env2 a)

deriving instance
         (Read (TyOf a), Read a, Read (ArrowTy a)) => Read (Env2 a)

deriving instance
         (Eq (TyOf a), Eq a, Eq (ArrowTy a)) => Eq (Env2 a)
...
emptyEnv2 :: Env2 a
emptyEnv2 = Env2 {vEnv = emptyTyEnv, fEnv = M.empty}
...
-- | unions for Env2
unionEnv2s :: [Env2 a] -> Env2 a
unionEnv2s envs = foldr unionEnv2 emptyEnv2 envs


-- union for Env2
unionEnv2 :: Env2 a -> Env2 a -> Env2 a
unionEnv2 (Env2 ve1 fe1) (Env2 ve2 fe2) =
  Env2 (M.union ve1 ve2) (M.union fe1 fe2)
...
class Expression e =>
      SimplifiableExt e ext
  where
  gInlineTrivExt :: M.Map Var e -> ext -> ext

type HasSimplifiable e l d
   = ( Show l
     , Out l
     , Show d
     , Out d
...
type HasSimplifiableExt e l d
   = (Show l, Out l, Show d, Out d, Simplifiable (PreExp e l d))
...
class Expression e =>
      SubstitutableExt e ext
  where
  gSubstExt :: Var -> e -> ext -> ext
  gSubstEExt :: e -> e -> ext -> ext

type HasSubstitutable e l d
   = ( Expression (e l d)
...
type HasSubstitutableExt e l d
   = (Eq d, Show d, Out d, Eq l, Show l, Out l, Substitutable (PreExp e l d))
...
type HasGeneric ex
   = (Generic ex, Generic (TyOf ex), Generic (ArrowTy (TyOf ex)))
...
newtype InterpM s e a =
  ...
  deriving newtype ( Functor
                   ...
                   , MonadState s
                   , MonadIO
                   , MonadWriter InterpLog
                   )

instance MonadFail (InterpM a b)
 where
  fail = error
...
class (Expression e, Expression ext) =>
      InterpExt s e ext
  where
  gInterpExt ::
       RunConfig
