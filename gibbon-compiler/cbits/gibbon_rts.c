...
long long global_init_biginf_buf_size = (4 * GB);
long long global_init_inf_buf_size = 1 * KB;
long long global_max_chunk_size = (1 * GB);

// Runtime arguments, values updated by the flags parser.
long long global_size_param = 1;
long long global_iters_param = 1;
char *global_bench_prog_param;
char *global_benchfile_param = NULL;
char *global_arrayfile_param = NULL;
long long global_arrayfile_length_param = -1;

// Number of regions allocated.
long long global_region_count = 0;

// Invariant: should always be equal to max(sym_table_keys)
SymTy global_gensym_counter = 0;
...
// Exponentiation
long long expll(IntTy base, IntTy pow) {
    if (base == 2) {
        return (1 << pow);
    } else {
        IntTy i, result = 1;
...
void init_bumpalloc() {
      ...
      printf("bump_alloc/init_bumpalloc DONE: heap_ptr = %p\n", bumpalloc_heap_ptr);
#endif
}
...
// Regular malloc mode:
void init_bumpalloc() {}
void *bump_alloc(long long n) { return malloc(n); }
void save_alloc_state() {}
void restore_alloc_state() {}
...
inline void *alloc_in_nursery(long long n) {
    if (! nursery_heap_ptr) {
        init_nursery();
    }
...
ArenaTy alloc_arena() {
  ...
  ar->ind = 0;
  ar->mem = malloc(global_max_chunk_size);
  ar->reflist = 0;
  return ar;
}
...
inline void insert_into_outset(CursorTy ptr, RegionTy *reg) {
...
inline void remove_from_outset(CursorTy ptr, RegionTy *reg) {
    uint outset_len = reg->reg_outset_len;
...
ChunkTy alloc_chunk(CursorTy end_old_chunk) {
    ...
    // See #110.
    if (newsize > global_max_chunk_size) {
        newsize = global_max_chunk_size;
    }
...
inline void bump_ref_count(CursorTy end_b, CursorTy end_a) {
    // Grab footers.
...
inline void *vector_nth(VectorTy *vec, IntTy i) {
...
inline VectorTy *vector_inplace_update(VectorTy *vec, IntTy i, void* elt) {
    void* dst = vector_nth(vec, i);
    memcpy(dst, elt, vec->vec_elt_size);
    return vec;
}

inline VectorTy *vector_copy(VectorTy *vec) {
...
inline VectorTy *vector_inplace_sort(VectorTy *vec, int (*compar)(const void *, const void*)) {
    void *start = vector_nth(vec, 0);
    qsort(start, vector_length(vec), vec->vec_elt_size, compar);
    return vec;
}

inline VectorTy *vector_sort(VectorTy *vec, int (*compar)(const void *, const void*)) {
    VectorTy *vec2 = vector_copy(vec);
    vector_inplace_sort(vec2, compar);
    return vec2;
}

inline VectorTy *vector_concat(VectorTy *vec) {
...
inline void vector_free(VectorTy *vec) {
    free(vec->vec_data);
    free(vec);
    return;
}

inline VectorTy *vector_merge(VectorTy *vec1, VectorTy *vec2) {
...
inline ListTy *list_alloc(IntTy data_size) {
    // ListTy *ls = ALLOC(sizeof(ListTy));
    ListTy *ls = bump_alloc(sizeof(ListTy));
...
inline BoolTy list_is_empty(ListTy *ls) {
    return ls->ll_next == NULL;
}

inline ListTy *list_cons(void *elt, ListTy *ls) {
    // void* data = ALLOC(ls->data_size);
    void* data = bump_alloc(ls->ll_data_size);
    if (data == NULL) {
    ...
    ListTy *res = bump_alloc(sizeof(ListTy));
...
inline void *list_head(ListTy *ls) {
    return ls->ll_data;
}

inline ListTy* list_tail(ListTy *ls) {
    return ls->ll_next;
}

inline void list_free(ListTy *ls) {
    free(ls->ll_data);
    free(ls);
    return;
}

inline ListTy *list_copy(ListTy *ls) {
