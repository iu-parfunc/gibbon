...
mkDeterministicTree :: Int -> Ast
mkDeterministicTree n = 
  if n > 0 then
    let m = 3 in
    if m == 0 then Not (mkDeterministicTree (n-1))
    else if m == 1 then And (mkDeterministicTree (n-1)) (mkDeterministicTree (n-1))
    else Or (mkDeterministicTree (n-1)) (mkDeterministicTree (n-1))
  else Val (1 == 0)
