      ...
      in case exp' of
           MkTailAndBlk tail blk0 ->
             let start = gensym
                 ...
                 blk2 = BlockCons start (copyPacked tail) (copyPacked blk0)

                 locals :: List Sym
                 locals = alloc_ll
             in ProgramC ty locals blk2

explicateTail :: ExpA -> TailAndBlk
explicateTail exp =
  case exp of
    SimplA simpl ->
      MkTailAndBlk (RetC (toExpC simpl)) BlockNil
    LetA2 v rhs bod ->
      let -- tail = explicateTail2 exp
          tb = MkTailAndBlk (RetC ReadC) BlockNil
      in tb
    LetA v rhs bod ->
      let rhs' = toExpC rhs
          bod' = explicateTail bod
      in case bod' of
           MkTailAndBlk tl blk ->
...
explicateTail2 :: ExpA -> TailC
explicateTail2 exp =
  case exp of
    SimplA simpl -> RetC (toExpC simpl)
    LetA2 v rhs bod ->
      let rhs' = toExpC rhs
          stm = AssignC v rhs'
          tail = explicateTail2 bod
          tail' = SeqC stm tail
      in tail'
