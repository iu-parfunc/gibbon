...
gibbon_main = do
  ...
  let octTree = buildOctree (sizeParam + 7) 17 0 64
  _ <- printsymIO (quote "Running pass sumMass (fold, uses=10): ")
  _ <- printsymIO (quote "NEWLINE")
  totMass <- iterateIO (\() -> sumMass octTree)
  ...
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  _ <- printsymIO (quote "Running pass countParticles (fold, uses=8): ")
  _ <- printsymIO (quote "NEWLINE")
  totParticles <- iterateIO (\() -> countParticles octTree)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  ...
  let clearedActive = countActive octTree'' 60
  return ((totMass, totEnergy, totActive, totParticles, bhPotential, fmmPot, scaledEnergy, clearedActive))
