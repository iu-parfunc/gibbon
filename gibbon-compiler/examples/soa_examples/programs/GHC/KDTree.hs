...
gibbon_main = do
  _ <- printsymIO (quote "Running program KDTree: ")
  _ <- printsymIO (quote "NEWLINE")
  let kdTree = buildKD (sizeParam + 22) 0
  _ <- printsymIO (quote "Running pass Find nearest Neighbour (fold_like, uses=13): ")
  _ <- printsymIO (quote "NEWLINE")
  dist <- iterateIO (\() -> nearestDist kdTree 1 2 3)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  _ <- printsymIO (quote "Running pass countInRange tight_box (fold_like, uses=13): ")
  _ <- printsymIO (quote "NEWLINE")
  inRangeCount <- iterateIO (\() -> countInRange kdTree (-20) 20 (-12) 12 (-7) 7)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  _ <- printsymIO (quote "Running pass sumMassInRange (fold_like, uses=14): ")
  _ <- printsymIO (quote "NEWLINE")
  massInRange <- iterateIO (\() -> sumMassInRange kdTree (-25) 25 (-20) 20 (-15) 15)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  _ <- printsymIO (quote "Running pass twoPointCorrelation bin_8_16 (fold_like, uses=11): ")
  _ <- printsymIO (quote "NEWLINE")
  corrCount <- iterateIO (\() -> twoPointCorrelation kdTree 0 0 0 8 16)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  _ <- printsymIO (quote "Running pass pointCloudNeighborhood (fold_like, uses=11): ")
  _ <- printsymIO (quote "NEWLINE")
  cloudCount <- iterateIO (\() -> pointCloudNeighborhood kdTree 0 0 0 24)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  _ <- printsymIO (quote "Running pass photonMappingBenchmark (fold_like, uses=12): ")
  _ <- printsymIO (quote "NEWLINE")
  photonHits <- iterateIO (\() -> photonMappingBenchmark kdTree 5 16 7 18)
  _ <- printsymIO (quote "End")
  _ <- printsymIO (quote "NEWLINE")
  return ((dist, inRangeCount, massInRange, corrCount, cloudCount, photonHits))

main = runGibbonMainIO gibbon_main

instance NFData KDTree
