-- GuardDivEdgeInt64: L (Factored).
-- Functions: numOf, denOf, mkL, xform, sumL.
-- Annotated: MayVectorize on xform; StoreScalarCounts on numOf, mkL.
data L = Cons Int64 Int64 Int64 Int64 L | Nil
{-# ANN type L "Factored" #-}

{-# ANN numOf "OPT:StoreScalarCounts" #-}
numOf :: Int64 -> Int64
numOf k =
  if k == 1 then (0 - 9223372036854775807) - 1 else
  if k == 2 then (0 - 9223372036854775807) - 1 else
  if k == 3 then 7 else
  if k == 4 then 0 - 7 else
  if k == 5 then 7 else
  if k == 6 then 0 - 7 else
  if k == 7 then 9223372036854775807 else 5
...
mkL k =
  if k <= 0
  then Nil
  else Cons (toInt64 (numOf k)) (toInt64 (denOf k)) 0 0 (mkL (k - 1))
