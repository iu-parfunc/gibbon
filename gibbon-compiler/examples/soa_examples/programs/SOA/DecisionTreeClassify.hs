-- DecisionTreeClassify: DTree (Factored).
-- Functions: buildTree, countNodes, countLeaves, treeDepth, sumImpurity,
-- sumSamples, countFeatureUses, countClass. ...
-- Annotated: StoreScalarCounts on buildTree.
{-# LANGUAGE BangPatterns #-}

module DTreeClassifyBench where
...
-- `feature` (an Int64 ADT field) already matches `nth`'s fixed-Int64
-- index parameter, so no conversion is needed at the call site; `val`
-- comes back Int (FeatureVec's element type), matching `threshold`
...
classifyDepthBatch :: DTree -> Int -> Int -> Int
classifyDepthBatch t fvSize i =
  if i <= 0
  then 0
  else
    let fv = generate fvSize (\j -> mod (j * 3 + i + sizeParam) 100) in
    let d = classifyDepth t fv 0 in
    d + classifyDepthBatch t fvSize (i - 1)
...
gibbon_main =
  ...
  let _ = printsym (quote "Running program Decision Tree Classify: ") in
  let _ = printsym (quote "NEWLINE") in
  -- Depth 14 (987 leaves, 1973 nodes): batched classification is
  -- superlinear in depth, so this file keeps the committed small
  -- tree. See this file's header for the sizing.
  let tree = buildTree (sizeParam + 14) in

  -- Structural analyses
  let _ = printsym (quote "Running pass classify Depth (fold, uses=4): ") in
  let _ = printsym (quote "NEWLINE") in
  let pdepth = iterate(classifyDepthBatch tree 32 250000) in
  let _ = printsym (quote "End") in
  let _ = printsym (quote "NEWLINE") in
  let _ = printsym (quote "Running pass classify Batch (fold, uses=5): ") in
  let _ = printsym (quote "NEWLINE") in
  -- Batched inference
  let batch = iterate(classifyBatch tree 32 1000000) in
  let _ = printsym (quote "End") in
  let _ = printsym (quote "NEWLINE") in
  (pdepth, batch)
