...
static IntTy *run_add1_kernel(const IntTy *in,
                              size_t length,
                              void (*kernel)(IntTy *, const IntTy *, size_t),
...
static NOINLINE NO_TREE_VECTORIZE void add1_scalar_kernel(IntTy *out, const IntTy *in, size_t length);
static NOINLINE void add1_auto_kernel(IntTy *out, const IntTy *in, size_t length);
static TARGET_SSE2 NOINLINE void add1_sse2_kernel(IntTy *out, const IntTy *in, size_t length);
static TARGET_AVX2 NOINLINE void add1_avx2_kernel(IntTy *out, const IntTy *in, size_t length);
...
static IntTy *run_add1_kernel(const IntTy *in,
    ...
    IntTy *out = (IntTy *) malloc(sizeof(IntTy) * length);
    if (out == NULL) {
        perror("malloc add1 output");
        return NULL;
    }

    if (record_time) {
        double start = now_seconds();
        kernel(out, in, length);
        double end = now_seconds();
...
static NOINLINE NO_TREE_VECTORIZE void add1_scalar_kernel(IntTy *out,
                                                          const IntTy *in,
                                                          size_t length) {
    for (size_t i = 0; i < length; i++) {
        out[i] = in[i] + 1;
...
static NOINLINE void add1_auto_kernel(IntTy *out,
                                      const IntTy *in,
                                      size_t length) {
    for (size_t i = 0; i < length; i++) {
        out[i] = in[i] + 1;
...
static TARGET_SSE2 NOINLINE void add1_sse2_kernel(IntTy *out,
                                                  const IntTy *in,
                                                  size_t length) {
    size_t i = 0;
...
static TARGET_AVX2 NOINLINE void add1_avx2_kernel(IntTy *out,
                                                  const IntTy *in,
                                                  size_t length) {
    size_t i = 0;
    if (sizeof(IntTy) == sizeof(int)) {
