...
#ifndef SCALAR_COUNT_SMOKE_LIST_LEN
#define SCALAR_COUNT_SMOKE_LIST_LEN 500000
#endif

#ifndef SCALAR_COUNT_SMOKE_INT_ONLY_REPEATS
#define SCALAR_COUNT_SMOKE_INT_ONLY_REPEATS 0
#endif
...
static void __attribute__((noinline,optimize("no-tree-vectorize")))
manual_scalar_zero_dcon_chunk(GibCursor out, uint64_t count)
{
    for (uint64_t i = 0; i < count; i++) {
        out[i] = 0;
    }
...
static void __attribute__((noinline,optimize("no-tree-vectorize")))
manual_scalar_copy_float_chunk(GibCursor in, GibCursor out, uint64_t count)
{
...
static volatile GibInt manual_int_only_sink = 0;

static double manual_time_int_chunk(manual_int_chunk_fn fn,
                                    GibCursor in,
                                    GibCursor out,
                                    uint64_t count,
                                    uint64_t repeats)
{
    ...
    for (uint64_t r = 0; r < repeats; r++) {
        fn(in, out, count);
        manual_int_only_sink += ((GibInt *) out)[(r * 9973) % count];
    }

    return (manual_now_seconds() - start) / (double) repeats;
}
                                         ...
                                         ManualLoopTiming *int_loop_timing)
{
    ...
    while (true) {
        ...
        uint64_t count =
            gib_scalar_count_footer_get((char *) int_count_footer);
        uint64_t float_count =
            gib_scalar_count_footer_get((char *) float_count_footer);
        ...
        if (use_vector_instructions) {
            ...
            manual_scalar_zero_dcon_chunk(out_dcon, count);
        }
        out_dcon += count;
        in_dcon += count;
        ...
        if (use_vector_instructions) {
            ...
            manual_scalar_copy_float_chunk(in_float, out_float, count);
        }
...
int main(int argc, char **argv)
{
    ...
    unsigned char tup_packed_1513 =
                   mkList(reg_cursor_ptr_1500, cursor_ptr_1501,
                          SCALAR_COUNT_SMOKE_LIST_LEN);
    ...
    size_t int_only_bytes =
        (size_t) SCALAR_COUNT_SMOKE_LIST_LEN * sizeof(GibInt);
    GibInt *int_only_in = (GibInt *) malloc(int_only_bytes);
    GibInt *int_only_out = (GibInt *) malloc(int_only_bytes);

    if (int_only_in == NULL || int_only_out == NULL) {
        fprintf(stderr, "int-only benchmark allocation failed\n");
        exit(1);
    }

    for (uint64_t i = 0; i < SCALAR_COUNT_SMOKE_LIST_LEN; i++) {
        int_only_in[i] = (GibInt) i;
    }

    double int_only_scalar_seconds =
        manual_time_int_chunk(manual_scalar_add1_int_chunk,
                              (GibCursor) int_only_in,
                              (GibCursor) int_only_out,
                              SCALAR_COUNT_SMOKE_LIST_LEN,
                              SCALAR_COUNT_SMOKE_INT_ONLY_REPEATS);
    double int_only_auto_seconds =
        manual_time_int_chunk(manual_auto_add1_int_chunk,
                              (GibCursor) int_only_in,
                              (GibCursor) int_only_out,
                              SCALAR_COUNT_SMOKE_LIST_LEN,
                              SCALAR_COUNT_SMOKE_INT_ONLY_REPEATS);
    double int_only_vectorized_seconds =
        manual_time_int_chunk(manual_vec_add1_int_chunk,
                              (GibCursor) int_only_in,
                              (GibCursor) int_only_out,
                              SCALAR_COUNT_SMOKE_LIST_LEN,
                              SCALAR_COUNT_SMOKE_INT_ONLY_REPEATS);
    double int_only_auto_over_scalar =
        int_only_auto_seconds > 0.0
            ? int_only_scalar_seconds / int_only_auto_seconds
            : 0.0;
    double int_only_vectorized_over_scalar =
        int_only_vectorized_seconds > 0.0
            ? int_only_scalar_seconds / int_only_vectorized_seconds
            : 0.0;
    ...
    printf("int_only_auto_seconds=%.9f\n", int_only_auto_seconds);
    printf("int_only_vectorized_seconds=%.9f\n",
           int_only_vectorized_seconds);
    printf("speedup_int_only_auto_over_scalar=%.3fx\n",
           int_only_auto_over_scalar);
    printf("speedup_int_only_vectorized_over_scalar=%.3fx\n",
           int_only_vectorized_over_scalar);
    printf("int_only_sink=%ld\n", manual_int_only_sink);
#endif
    printf("sums_match=%s\n",
           loop_scalar_sum == scalar_sum &&
           loop_scalar_sum == indir_loop_scalar_sum &&
           loop_scalar_sum == indir_auto_sum &&
           loop_scalar_sum == indir_vector_sum
               ? "yes" : "no");
