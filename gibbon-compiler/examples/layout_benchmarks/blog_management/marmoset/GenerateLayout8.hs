...
mkBlogs_layout8 contentfiles tagfiles length = 
   if length < 0 then End 
      ...
      let select  = (mod length 10)
          ...
          ft      = fromMaybe def (nth_plist tagfiles Nothing select)
         in Layout8 (Content (Plain (mkRandomInlineList 100))) (mkBlogs_layout8 contentfiles tagfiles (length - 1)) (ID (10 - (mod length 10))) (Author (getRandomString (mod rand 9))) (Date (getRandomString (mod rand 9))) (Header (getRandomString (mod rand 9))) (TagList (mkSomeTags 10)) 
