...
mkBlogs_layout4 contentfiles tagfiles length =
   if length < 0 then End
      ...
      let select  = (mod length 10)
         ...
         in Layout4 (TagList (mkSomeTags 2)) (Content (Plain (mkRandomInlineList 1000))) (mkBlogs_layout4 contentfiles tagfiles (length - 1)) (Header (getRandomString (mod rand 9))) (ID (10 - (mod length 10))) (Author (getRandomString (mod rand 9))) (Date (getRandomString (mod rand 9)))
