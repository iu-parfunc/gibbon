...
mkBlogs_layout6 contentfiles tagfiles length =
   if length < 0 then End
      ...
      let select  = (mod length 10)
         ...
         in Layout6 (Header (getRandomString (mod rand 9))) (ID (10 - (mod length 10))) (Author (getRandomString (mod rand 9))) (Date (getRandomString (mod rand 9))) (Content (Plain (mkRandomInlineList 1000))) (mkBlogs_layout6 contentfiles tagfiles (length - 1)) (TagList (mkSomeTags 2))
