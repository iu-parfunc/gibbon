...
CursorCursorCursorProd buildTree(CursorTy end_out_reg, CursorTy out_cur, Box *box, Array_MassPoint *mpts);
...
void mapCalcAccel(Array_Point2D *dst, Array_MassPoint *mpts, CursorTy tr) {
    ...
    for (idx = 0; idx < mpts->size_masspoints; idx++) {
        ...
        calcAccel(&dst->points[idx], &mp, tr);
    }
    dst->size_points = idx;
}
...
void __main_expr() {
    ...
    for (int i = 0; i < global_iters_param; i++) {
        tree = buildTree(end_reg,cur,&box, mpts);
