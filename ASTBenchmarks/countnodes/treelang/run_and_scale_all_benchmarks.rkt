...
(define target-time (make-parameter 0.0))
...
(fprintf csv "NAME, VARIANT, ARGS, ITERS, NODECOUNT\n")
...
(for ([relative (in-list all-files)])
  ...
  (printf "\n\nBenchmark(~a): Counting nodes in file ~a ...\n" fileNum relative)
  ...
  (let loop ([iters 1])
    (define-values (cn cpu real gc)
      (begin (collect-garbage) ;(collect-garbage)(collect-garbage)
             (time-apply (lambda () (for/list ([_ (in-range iters)])
                                      (countnodes ast)))
                         '())))
    ...
    (if (>= batchseconds (target-time))
        (let ((meantime (exact->inexact (/ batchseconds iters))))
          ...
          (fprintf csv "countnodes, treelang-racket, ~a, ~a, ~a\n"
                   relative iters (caar cn))
          (flush-output csv)
