...
the space at each level of the tree, when the number of the points in the subtree is one a leaf node that stores one point of in the space is constructed .
...
We evaluate two version of the point correlation traversal that operate on a kd-tree representation,int out and tree out, int out version
produces an integer as the output of the traversal while  tree out version  produces a tree that have the same data of the input tree except
that that some counters in the lead nodes are incremented, these are the counter in the leaf nodes that correlate with the input point.Unlike 
the rest of the benchmarks evaluated in this paper, the int out version does not need to traverse the whole tree this
will requires that the packed version stores the address of the right child for each internal node, this is
needed because the left nodes is not always traversed and when that happens that address of the right child wont be available and is read form 
additional field.Tree out version does not need that additional field, although the computation might  truncate at some points during the traversal, 
the whole tree always need to be traversed to copy the data from the input tree to the output tree, and the right child address is always available.
