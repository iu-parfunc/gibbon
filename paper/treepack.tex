...
\Red{We demonstrate speedups from ABC to XYZ}
...
%  prototype language implementation based on this
 ...
 to be \Red{several times} faster than standard techniques.
...
execution in Racket).
%
\Red{We see that our compiler generated code that is competitive with, and
...
\begin{figure}[t]
  ...
  \begin{minipage}{1.04\textwidth}
    \begin{minipage}{.49\textwidth}
      \centering
      \includegraphics[width=3.3in]{./figs/slowdown_leaves_build.pdf}
    \end{minipage}
    \begin{minipage}{.49\textwidth}
      \includegraphics[width=3.3in]{./figs/slowdown_leaves_add1.pdf}
    \end{minipage}
  \end{minipage}
  \vspace{-4mm}
...
the performance of two classes of tree walk (a fold and a map)
on full Racket core syntax, which is excerpted in \cref{fig:racket-core}.
%
...
%
Hence, we expect the results of these two traversals to be representative.
...
%
As input, we use a variety of different ASTs \Red{drawn from this input set},
...
ranging from XXX bytes to YYY bytes. This range of programs and sizes allows us
