...
\paragraph*{Ensuring complete traversal} Our \il{add1} function is
...
Thus we design this pass as a \Red{traditional program analysis} that iterates
to a fixed point.
%
...
Haskell, and generating code via GCC 5.3.  \Red{(What version is Laith using!!?)}
%
...
will potentially duplicate work (rather than copying) if a packed result of a
function call is used in multiple places, or when a function returns multiple
packed results.

\paragraph*{Packed \& Pointer, malloc \& bumpalloc}

The \treelang C backend supports multiple modes of code generation, which we
compare in the next section.  The first distinction is between:
...
\item {\bf pointer}: use traditional C struct representations.  This mode shares
  the front and back of the compiler with packed mode.  But it skips the
  invasive transformations of the source program which introduce cursors and
  packed representations.  Rather, it uses a traditional pointer graph of heap
  objects to represent all data.
\end{itemize}
...
future we will change this implementation to use a linked list of blocks


Then, within the {\bf pointer}
...
recursion---which are the exact emphasis of \treelang.
...
All benchmarks were conducted on a \Red{XYZ}.
...
overall. Figures~\ref{fig:copyprop} and~\ref{fig:subst} present the results
...
\section{Evaluation of Extensions}
