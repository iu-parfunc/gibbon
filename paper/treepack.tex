...
and primitive operations.  Conditionals are included to avoid the need for \gramwd{Bool}
to be packed data (because \gramwd{case} operates on packed data only).
%
...
explicitly encode facts about how to reach the end of a given location.
%
Namely:

\begin{itemize}
\item One field's end becomes its successor's start.  This becomes a
  binding \il{let start(y) = end(x)}.
