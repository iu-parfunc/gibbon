...
\mv{Contrast with papers by Mytkowicz, etc. required for ECOOP
  acceptance.  Is Milind handling this?  Seems like a big difference
  is that their work is focused on using SIMD operations in tree
  traversals.}
...
Even though cursors are internal to the compiler, rather than exposed
to the user, we use a typing discipline resembling {\em session
  types}~\cite{SessionTypesHonda} to ensure their correct
handling---specifically that data is read from and written to buffers
in the correct order.
In addition to an $\ENDOF$ cursor, which isn't directly usable for
anything, we also have $\HAS{}$ cursors for reading and $\NEEDS{}$
cursors for writing, as we will describe further in
\cref{sec:cursorize}.
...
\begin{code}
  ...
  add1 tr = case tr of
      Leaf n ->  let start(n) = start(tr) + tag in
                 let end(n)   = start(n) + offset in
                 let end(tr)  = end(n) in
                 (end(tr), Leaf n+1)
      Node x y ->  let start(x) = start(tr) + tag in
...
\begin{code}
  ...
  add1'' cin cout = let (cin, tag) = read cin in
    switch tag of
      Leaf -> let cout = write cout Leaf in
              let (cin, n) = read cin in
              let cout = write cout (n + 1) in
              (cin, finish cout)
      Node -> let cout = write cout Node in
...
%%       (NULLSYM))
  ...
  \caption{Excerpt of Racket Core AST definition in Gibbon., which 
   ...
   There are nine datatypes total.}
