...
often run several times faster than their pointer-based counterparts.
...
analysis inferring data flow, and a program synthesis step for
creating missing traversals. We evaluate our compiler on several
simple tree transformations over a real-world dataset of source code.
...
\item We present a prototype language implementation, dubbed \treelang,
  based on this transformation that can compile a range of tree transforms
  to be several times faster than standard
  techniques~\cref{sec:eval-compiler}.
...
simplest version: binary trees with
integer leaves.
%
