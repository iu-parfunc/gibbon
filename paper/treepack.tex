...
interesting tree-manipulating, such as common compiler passes.

The syntax of \treelang is given in Figure~\cref{fig:source}---it is
...
conditionals, and primitive operations. The only base values
provided are integers and booleans.

\note{Finally, we provide dictionaries to support ...}
...
for expressing tree-shaped data and transformations on it is present. 
...
Haskell, and generating code via GCC 5.3.  \rn{(What version is Laith using!!?)}
%
...
Finally, in \cref{sec:eval-extensions}, we look ahead to what a future compiler
...
simple binary trees. We implement 3 operations:
...
Because \treelang{} does not currently support a packed representation extended with layout information, our evaluation uses hand-written packed implementations (in C) that include that layout information, mimicking what would be produced by a backend that understands indirections. We evaluate two benchmarks: a {\em parallelization} microbenchmark (Section~\ref{sec:eval-parallel}) that uses pointer-style indirections to distribute the traversal of a tree, and an implementation of {\em two-point correlation} (Section~\ref{sec:eval-kdtree}) that uses ropes-style indirections.
...
\note{How much of this do we want to leave in?
}
