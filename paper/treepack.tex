...
One efficient way of doing that is to use kd-trees to store to represent the set of points\cite{capnproto}. 
...
the space at each level of the tree,when the number of the points in the subtree is one a  leaf node is constructed that stores one point of in the space.

In  a naive implementation point correlation need to be executed at leaf nodes for each input point, however kd-trees 
allow the it to  prune early by storing at each internal node the boundaries within which all descendent points lies,
and using that information to skip a subtree if the given point is far enough from the boundaries.
