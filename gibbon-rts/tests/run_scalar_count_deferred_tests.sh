#!/usr/bin/env bash
# Deferred scalar counts: runs each scenario with the per-element bump AND
# with the deferred counter and compares the resulting footer chains.  Builds gibbon_rts.c together
...
BIN=$TMP/scalar_count_deferred_test
$CC -O1 -g -std=gnu11 -D_GIBBON_VERBOSITY=1 -D_GIBBON_GENGC=0 \
    ...
    -o "$BIN" "$ROOT/gibbon-rts/tests/scalar_count_deferred_test.c" \
