...
RegionTy *alloc_region(IntTy size) {
    ...
    CursorTy start = (CursorTy) ALLOC_PACKED(total_size);
    CursorTy end = start + size;

    RegionTy *reg = (RegionTy*) malloc(sizeof(RegionTy));
    reg->refcount = 0;
    reg->start_ptr = start;
    reg->outset = NULL;
...
void sumFoo_bfs(std::vector<CursorTy> Blocks, IntTy* summedBuffer) {

    std::vector<CursorTy> nextBlock;
    ...
    for (size_t i = 0; i < Blocks.size(); ++i){
        ...
        if (tag == 0) {
            ...
            // A
            CursorIntProd returnedBuffer = sumFoo_seq(in);       
            sum += returnedBuffer.field1;
            ...
            nextBlock.push_back(next);
            nextBlock.push_back(ran1);
            nextBlock.push_back(ran2);

            mylock.unlock();

        } else if (tag == 4) {

            CursorIntProd buffer = sumFoo_seq(in);
            sum += buffer.field1;
    ...
    if(!nextBlock.empty()){
        sumFoo_bfs(nextBlock, summedBuffer);
    }
...
int main (int argc, char** argv) {
    ...
    print_size("Size of scratchpad (RAN's):", (foo.field2 - r2));
    ...
    IntTy sum_bfs = 0;
    startBlock.push_back(r1); //initially only r1 in the number of blocks

    for (int i = 0; i < iters; i++) {
        clock_gettime(CLOCK_MONOTONIC_RAW, &begin_timed);
        sumFoo_bfs(startBlock, &sum_bfs);
        clock_gettime(CLOCK_MONOTONIC_RAW, &end_timed);
        total_sum += sum_bfs;
        selftimed = difftimespecs(&begin_timed, &end_timed);
        nums[i] = selftimed;
