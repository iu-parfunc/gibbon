...
# Plot the median runtime vs percentage of fields used for each (k, list_size) tuple value with 95% confidence intervals
unique_tuples = df.groupby(['k', 'list_size']).size().reset_index().rename(columns={0:'count'})
...
for _, row in unique_tuples.iterrows():
    ...
    subset_tuple = df[(df['k'] == k) & (df['list_size'] == list_size)]
    
    for function in subset_tuple['function'].unique():
        subset_function = subset_tuple[subset_tuple['function'] == function]
        plt.errorbar(subset_function['median'], subset_function['percentage_used'], 
    ...
    plt.ylabel("Percentage of Fields Used in Cons Style List (%)")
    plt.xlabel("Median Runtime")
    plt.title(f"Performance Comparison: Median Runtime vs Percentage of Fields Used (k: {k}, List Size: {list_size})")
    plt.legend()
    plt.grid(True)
    plt.show()
