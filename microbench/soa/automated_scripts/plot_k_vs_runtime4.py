...
columns = ["k", "l", "list_size", "function", "average", "median", "lower_bound", "upper_bound", "total_instructions", "l2_cache_misses", "l3_cache_misses"]
...
for row in data:
    ...
    if len(cols) >= 11:
        # Extract the values for average, median, lower_bound, upper_bound, total_instructions, l2_cache_misses, and l3_cache_misses
        ...
        l3_cache_misses = cols[10].strip("('),")
        
        # Append the processed row to the list
        processed_data.append([cols[0], cols[1], cols[2], cols[3], average, median, lower_bound, upper_bound, total_instructions, l2_cache_misses, l3_cache_misses])
...
df["l3_cache_misses"] = pd.to_numeric(df["l3_cache_misses"].str.replace("'", ""))
...
# Plot runtime, total instructions, L2 cache misses, and L3 cache misses vs k values for each (l, list_size) tuple value with 95% confidence intervals as subplots
...
for _, row in unique_tuples.iterrows():
    ...
    for function in subset_tuple['function'].unique():
        ...
        axs[2].errorbar(subset_function['k'], subset_function['l2_cache_misses'], 
                        fmt=markers[function], linestyle=':', color=colors[int(l)], label=f"{function} - L2 Cache Misses")
        
        # Plot L3 cache misses vs k values
        axs[3].errorbar(subset_function['k'], subset_function['l3_cache_misses'], 
                        fmt=markers[function], linestyle='-.', color=colors[int(l)], label=f"{function} - L3 Cache Misses")
    ...
    axs[3].set_xlabel("k Values")
    axs[3].set_ylabel("L3 Cache Misses")
    axs[3].set_title(f"Performance Comparison: L3 Cache Misses vs k Values (l: {l}, List Size: {list_size})")
