...
l = 1
list_size = 300000000
functions_soa = ["add1RecursiveInPlace", "add1RecursiveOutOfPlace", "add1IterativeInPlace", "add1IterativeOutOfPlace", "add1IterativeOpt"]
...
special_options_keys = ["add1IterativeOpt-vecoff", "add1IterativeOpt-256bitvec"]
...
def compile_special_gcc():
    out_file = out_file_template_soa.format("add1IterativeOpt")
    
    for i, options in enumerate(special_options_gcc):
        exec_file = f"executable_gcc_special_{i}"
...
def compile_special_clang():
    ...
    for i, options in enumerate(special_options_clang):
        exec_file = f"executable_clang_special_{i}"
...
for i, options in enumerate(special_options_gcc):
    exec_file_gcc = f"executable_gcc_special_{i}"
    exec_file_clang = f"executable_clang_special_{i}"
    
    times, instructions, l2_misses, l3_misses = run_executable(exec_file_gcc, "add1IterativeOpt")
    data_special_gcc[special_options_keys[i]] = {
        "times": times,
        "instructions": instructions,
        "l2_misses": l2_misses,
        "l3_misses": l3_misses,
    ...
    times, instructions, l2_misses, l3_misses = run_executable(exec_file_clang, "add1IterativeOpt")
    data_special_clang[special_options_keys[i]] = {
        "times": times,
        "instructions": instructions,
        "l2_misses": l2_misses,
        "l3_misses": l3_misses,
...
for function in special_options_keys:
    table_data_gcc_soa.append([
        function,
        data_special_gcc[function]["median_time"],
        data_special_gcc[function]["median_instructions"],
        data_special_gcc[function]["median_l2_misses"],
        data_special_gcc[function]["median_l3_misses"]
    ])

    table_data_clang_soa.append([
        function,
        data_special_clang[function]["median_time"],
        data_special_clang[function]["median_instructions"],
        data_special_clang[function]["median_l2_misses"],
        data_special_clang[function]["median_l3_misses"]
    ])
...
for key, cell in table.get_celld().items():
    cell.set_height(0.07)  # Increase row height
